        ...
        while ( !$obRecordSet->eof() and $inCont < $this->obPaginacao->getMaxLinhas() ) {
            ...
            if ($inNumDados) {
                ...
                foreach ($arDado as $arObDado) {
                    ...
                    $arObDado = version_compare(phpversion(), '5.0') < 0 ? $arObDado : clone($arObDado);

                    if (strtolower(get_class($arObDado))=='dadocomponente') {
                        //FIXME : correcao temporaria para php4 e 5
                        $arObDado->obComponenteLista = version_compare(phpversion(), '5.0') < 0 ? $arObDado->obComponenteLista : clone($arObDado->obComponenteLista);
                ...
                if ( count( $arAcao ) ) {
                    foreach ($arAcao as $obAcao) {
                        $obAcao = version_compare(phpversion(), '5.0') < 0 ? $obAcao : clone($obAcao);
