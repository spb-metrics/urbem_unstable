...
while ( !$rsDocumento->eof() ) {
    ...
    $arDocumentos[] = version_compare(phpversion(), '5.0') < 0 ? $obChkDocumento : clone($obChkDocumento);
    $rsDocumento->proximo();
}
...
while ( !$rsAtributoProtocolo->eof() ) {
    ...
    $arAtributos[] = version_compare(phpversion(), '5.0') < 0 ? $obChkAtributo : clone($obChkAtributo);
