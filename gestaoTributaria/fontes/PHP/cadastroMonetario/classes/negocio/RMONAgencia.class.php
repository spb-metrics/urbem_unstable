...
/**
    ...
    * $Id: RMONAgencia.class.php 59612 2014-09-02 12:00:51Z gelson $
...
function incluirAgencia($boTransacao = "")
{
    ...
    if ( !$obErro->ocorreu() ) {
        ...
        if ( !$obErro->ocorreu() ) {
            if ( !$rsBanco->eof() ) {
                $obErro = $this->verificaNomeAgencia();
                if ( !$obErro->ocorreu() ) {
...
/**
    ...
    * @return Object Objeto Erro
*/
function verificaNomeAgencia($boTransacao = "")
{
    $stAgencia = Sessao::read( "stNumAgencia" )?Sessao::read( "stNumAgencia" ):$this->getNumAgencia();
    $stFiltro  = " WHERE upper(nom_agencia) = '" . strtoupper ( $this->getNomAgencia() ) . "' ";
    $stFiltro .= " AND num_agencia <> '".$stAgencia."' ";
    $obErro = $this->obTMONAgencia->recuperaTodos( $rsRecordSet, $stFiltro, $stOrdem, $boTransacao );
    if ( $rsRecordSet->getNumLinhas() > 0 ) {
        $obErro->setDescricao("Agência já cadastrada no Sistema! (". $this->stNomAgencia .")");
    }
