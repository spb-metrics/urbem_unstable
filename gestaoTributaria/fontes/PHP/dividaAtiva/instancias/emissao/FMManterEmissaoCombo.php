 ...
 * $Id: FMManterEmissaoCombo.php 59612 2014-09-02 12:00:51Z gelson $
...
foreach ($_REQUEST as $valor => $key) {
    if ( preg_match("/^[a-z]boEmitir_[0-9]/",$valor) ) {
        ...
        if ($_REQUEST["cmbModelo"] == "Certidão de Remissão DA") {
            ...
            for ($inX=0; $inX<$inTamanhoLista; $inX++) {
                if ( ( $arLista[$inX][3] == $arKey[3] ) && ( in_array( $arLista[$inX][0], explode( "$$", $arKey[0] ) ) ) ) {
                    if ( $arKey[10] || ( $_REQUEST["cmbModelo"] == "Termo Inscrição DA" ) || ( $_REQUEST["cmbModelo"] == "Notificação DA" ) || ( $_REQUEST["cmbModelo"] == "Certidão DA" ) ) {
        ...
        if (!$boJaNaLista) {
            ...
            for ( $inX=0; $inX<count($arTMP1); $inX++ ) {
                if ($arTMP1[$inX] == $_REQUEST["cmbModelo"]) {
                    $boJaNaLista = true;
                    break;
                }
            }
...
foreach ($arLista as $valor => $arKey) {
        ...
        } elseif ($arKey[0] == "termoInscricao.agt" || $arKey[0] == "certidaoDivida.agt") { //era o dado que vem do campo agt
