...
*/
        ...
        Sessao::getTransacao()->setMapeamento( $obTDATDividaParcelamento );
            ...
            $stTipoDasInscricoes = $arDadosSessao[0]["inscricao_tipo"];
            $inNumeroDasInscricoes = $arDadosSessao[0]["inscricao"];
            $boTipoDasInscricoesIgual = true;
            $nuTotalReducaoCredito = 0;
            for ($inX=0; $inX<$inTotalDados; $inX++) {
                if ($boTipoDasInscricoesIgual) {
                    if ( ($stTipoDasInscricoes != $arDadosSessao[$inX]["inscricao_tipo"]) || ($inNumeroDasInscricoes != $arDadosSessao[$inX]["inscricao"]) ) {
                        $boTipoDasInscricoesIgual = false;
                    }
                }
            ...
            for ($inA=0; $inA<$arInscricoesAgrupadasSessao["total_creditos_agrupados"]; $inA++) { //inserindo os calculo (1 calculo por credito)
                ...
                if ($boTipoDasInscricoesIgual && $inNumeroDasInscricoes) {
                    if ($stTipoDasInscricoes == "imobiliaria") {
                        $stFiltroImovel = " WHERE inscricao_municipal = ".$inNumeroDasInscricoes;
                        ...
                        $obTARRImovelCalculo->setDado ( 'inscricao_municipal', $inNumeroDasInscricoes );
                        $obTARRImovelCalculo->setDado ( 'timestamp', $rsListaTimestampImovel->getCampo("timestamp") );
                        $obTARRImovelCalculo->inclusao();
                    }else
                    if ($stTipoDasInscricoes == "economica") {

                        $stFiltroEconomico = " WHERE inscricao_economica = ".$inNumeroDasInscricoes;
                        ...
                        $obTARRCadastroEconomicoCalculo->setDado ( 'inscricao_economica', $inNumeroDasInscricoes );
