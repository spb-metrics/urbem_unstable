...
/**
    ...
    * $Id: PRManterAtividade.php 59612 2014-09-02 12:00:51Z gelson $
...
switch ($stAcao) {
    ...
    case "alterar":
        ...
        if ($_REQUEST["inCodigoNivel"] > 1) {
            $arTMP = explode( ".", $_REQUEST["stValorReduzido"] );
            $stValor = $arTMP[ $_REQUEST["inCodigoNivel"]-1 ];
        }else
            $stValor = $_REQUEST["stValorReduzido"];

        $obRCEMAtividade->setValor              ( preg_replace( "/^0*/", "",trim( $stValor ) ) );
        $obRCEMAtividade->setAliquota           ( $_REQUEST["flAliquota"] );
        $obRCEMAtividade->setValorComposto      ( $_REQUEST["stChaveServico"]     );
        if ($_REQUEST["boUltimoNivel"]) {
            if ($_REQUEST['stChaveCnae']) {
                $obRCEMAtividade->addAtividadeCnae();
                $obRCEMCnae->setValorCompostoCnae( $_REQUEST['stChaveCnae'] );
                $obRCEMCnae->listarCnae($rsCnae);
                $inCodCnae =  $rsCnae->getCampo("cod_cnae");
        ...
        if ( !$obErro->ocorreu() ) {
            sistemaLegado::alertaAviso($pgList,"Nome Atividade: ".$_REQUEST['stNomeAtividade'],"alterar","aviso", Sessao::getId(), "../");
        } else {
