...
/**
    ...
    * $Id: RCEMAtividade.class.php 59612 2014-09-02 12:00:51Z gelson $
                    ...
                    if ( !$obErro->ocorreu() ) {
                        if ( is_object($this->roUltimoCnae) ) {
                            if ( $this->roUltimoCnae->getCodigoCnae() ) {
                                ...
                                if ( $rsAtividadeCnae->getNumLinhas() > 0 ) {
                                    $obErro = $this->roUltimoCnae->alterarAtividadeCnae( $boTransacao );
                                } else {
                                    $obErro = $this->roUltimoCnae->incluirAtividadeCnae( $boTransacao );
                                }
                            } else {
                                $obErro = $this->roUltimoCnae->excluirAtividadeCnae( $boTransacao );
                            }
                        ...
                        if ( !$obErro->ocorreu() ) {
                            if ( count( $this->arRCEMProfissao ) ) {
                                $obErro = $this->alterarAtividadeProfissao( $boTransacao );
                            } else {
                                $this->addAtividadeProfissao();
                                $obErro = $this->roUltimaProfissao->excluirAtividadeProfissao( $boTransacao );
                            }
                            ...
                            if ( !$obErro->ocorreu() ) {
                                if ( count( $this->arRCEMElemento ) ) {
                                    $obErro = $this->alterarAtividadeElemento( $boTransacao );
                                } else {
                                    $this->addAtividadeElemento();
                                    $obErro = $this->roUltimoElemento->excluirElementoAtividade( $boTransacao );
                                }
...
{
    $obErro = $this->roUltimaProfissao->obTCEMAtividadeProfissao->recuperaTodos( $rsAtividadeProfissao );
    if ( !$obErro->ocorreu() ) {
...
{
    $obErro = $this->roUltimoElemento->obTCEMElementoAtividade->recuperaElementoAtividade( $rsElementoAtividade );
    if ( !$obErro->ocorreu() ) {
