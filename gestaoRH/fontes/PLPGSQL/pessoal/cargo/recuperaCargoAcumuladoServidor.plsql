...
BEGIN
    ...
    INNER JOIN (SELECT contrato_servidor_funcao.cod_contrato
                  ...
                  FROM pessoal'||stEntidade||'.contrato_servidor_funcao
                 WHERE contrato_servidor_funcao.timestamp <= (ultimoTimestampPeriodoMovimentacao('||inCodPeriodoMovimentacao||','''||stEntidade||'''))::timestamp
              GROUP BY contrato_servidor_funcao.cod_contrato
