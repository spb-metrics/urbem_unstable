...
/**
    ...
    * Casos de uso: uc-04.05.24
    ...
    $Id: PRConcederDecimo.php 59612 2014-09-02 12:00:51Z gelson $
*/
...
switch ($stAcao) {
    case "inserir":
        if ( $rsContratos->getNumLinhas() <= 0 ) {
            ...
            switch ($_REQUEST['stTipoFiltro']) {
                case "reg_sub_fun_esp":
                    $obTFolhaPagamentoConcessaoDecimo->setDado( "inCodRegime", implode(",",$_REQUEST["inCodRegimeSelecionadosFunc"]) );
                    $obTFolhaPagamentoConcessaoDecimo->setDado( "inCodSubDivisao", implode(",",$_REQUEST["inCodSubDivisaoSelecionadosFunc"]) );
                    $obTFolhaPagamentoConcessaoDecimo->setDado( "inCodCargo", implode(",",$_REQUEST["inCodFuncaoSelecionados"]) );
                    if (is_array($_REQUEST["inCodEspecialidadeSelecionadosFunc"])) {
                        $obTFolhaPagamentoConcessaoDecimo->setDado( "inCodEspecialidade", implode(",",$_REQUEST["inCodEspecialidadeSelecionadosFunc"]) );
                    }
                    break;

                case "lotacao":
                    $obTFolhaPagamentoConcessaoDecimo->setDado( "inCodOrgao", implode(",",$_REQUEST["inCodLotacaoSelecionados"]) );
                    break;

                case "local":
                    $obTFolhaPagamentoConcessaoDecimo->setDado( "inCodLocal", implode(",",$_REQUEST["inCodLocalSelecionados"]) );
                    break;
            }
        ...
        if ($boContinuar == "true") {
            ...
            while (!$rsContratos->eof()) {
                ...
                $obTFolhaPagamentoConcessaoDecimo->setDado("desdobramento",$_POST['stDesdobramento']);
                $obTFolhaPagamentoConcessaoDecimo->setDado("cod_periodo_movimentacao",$rsUltimaMovimentacao->getCampo("cod_periodo_movimentacao"));
                $obTFolhaPagamentoConcessaoDecimo->setDado("folha_salario",$_POST['boPagEmFolhaSalario']);
                ...
                $stFiltro .= "   AND desdobramento = '".$_POST['stDesdobramento']."'";
                $obErro = $obTFolhaPagamentoConcessaoDecimo->recuperaTodos($rsConcessao,$stFiltro,'',$boTransacao);
                if ($obErro->ocorreu()) {
                    break;
                }

                if ($_POST['stDesdobramento'] == "A") {
                    if ( $rsConcessao->getNumLinhas() == -1 ) {
                        $nuPercentualPagamento = str_replace(".","",$_POST['nuPercentualPagamento']);
                        ...
                        $obTFolhaPagamentoConfiguracaoAdiantamento->setDado("vantagens_fixas"           ,$_POST['boGerarSomenteVantagem']);
                        ...
                        $obFFolhaPagamentoGeraRegistroDecimo->setDado("desdobramento"           ,$_POST['stDesdobramento']);
    ...
    case "cancelar":
        ...
        switch ($_REQUEST['stTipoFiltro']) {
           case "lotacao":
                $arCodLotacao = ( is_array($_REQUEST['inCodLotacaoSelecionados']) ) ? $_REQUEST['inCodLotacaoSelecionados'] : Sessao::read('arCodLotacao');
                if ( is_array($arCodLotacao) ) {
                    Sessao::write('arCodLotacao',$arCodLotacao);
                    $stValoresFiltro = implode(",",$arCodLotacao);
                }
           case "local":
                $arCodLocal = ( is_array($_REQUEST['inCodLocalSelecionados']) ) ? $_REQUEST['inCodLocalSelecionados'] : Sessao::read('arCodLocal');
                if ( is_array($arCodLocal) ) {
                    Sessao::write('arCodLocal',$arCodLocal);
                    $stValoresFiltro = implode(",",$arCodLocal);
                }
           case "reg_sub_fun_esp":
                $arCodRegime = ( is_array($_REQUEST['inCodRegimeSelecionadosFunc']) ) ? $_REQUEST['inCodRegimeSelecionadosFunc'] : Sessao::read('arCodRegime');
                $arCodSubDivisao = ( is_array($_REQUEST['inCodSubDivisaoSelecionadosFunc']) ) ? $_REQUEST['inCodSubDivisaoSelecionadosFunc'] : Sessao::read('arCodSubDivisao');
                $arCodFuncao = ( is_array($_REQUEST['inCodFuncaoSelecionados']) ) ? $_REQUEST['inCodFuncaoSelecionados'] : Sessao::read('arCodFuncao');
                $arCodEspecialidade = ( is_array($_REQUEST['inCodEspecialidadeSelecionadosFunc']) ) ? $_REQUEST['inCodEspecialidadeSelecionadosFunc'] : Sessao::read('arCodEspecialidade');
            ...
            default:
                ...
                $obErro = deletarConcessaoDecimo($_REQUEST['inCodContrato'],$rsUltimaMovimentacao->getCampo("cod_periodo_movimentacao"),$boTransacao);

                if (!$obErro->ocorreu()) {
                    //Recalculo do contrato
                    $arContratos[] = array("cod_contrato"=>$_REQUEST['inCodContrato']);
                    $rsContratos = new recordset;
                    $rsContratos->preenche($arContratos);
                    $obErro = recalcularSalario($rsContratos,$boTransacao);
                }
