...
foreach (Sessao::read("arEmissaoEmpenho") as $arEmissaoEmpenho) {
    if ($arEmissaoEmpenho["red_dotacao"] != "") {
        ...
        $obAutorizacaoEmpenho->obROrcamentoEntidade->setCodigoEntidade( Sessao::getCodEntidade() );
        ...
        if ($rsAutorizacao->getCampo("data_autorizacao") == "") {
            ...
            $stFiltro .= "   AND e.cod_entidade = ".Sessao::getCodEntidade();
        ...
        if (array_key_exists('selecionadas', $arAssinaturas)) {
            ...
            if ( isset($arAssinaturas) && count($arAssinaturas['selecionadas']) > 0 ) {
                ...
                $obTEmpenhoAutorizacaoEmpenhoAssinatura->setDado( 'cod_entidade', Sessao::getCodEntidade() );
        ...
        if (Sessao::read("origem") == "d") {
            ...
            $obTDiariasDiariaEmpenho->setDado("cod_entidade",Sessao::getCodEntidade());
...
if ( !$obErro->ocorreu() ) {
    ...
    $stCampos  =  "?".Sessao::getId()."&stAcao=imprimir&stCaminho=".$stCaminho."&inCodEntidade=".Sessao::getCodEntidade();
