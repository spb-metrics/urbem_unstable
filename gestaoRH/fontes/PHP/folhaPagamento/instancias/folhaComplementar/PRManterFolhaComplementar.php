...
switch ($stAcao) {
    case "abrir":
        ...
        $obErro = $obRFolhaPagamentoPeriodoMovimentacao->roRFolhaPagamentoFolhaComplementar->abrirFolhaComplementar();
    ...
    case "fechar":
        //SistemaLegado::BloqueiaFrames();
        $obRFolhaPagamentoPeriodoMovimentacao->setCodPeriodoMovimentacao($_REQUEST['inCodPeriodoMovimentacao']);
        $obErro = $obRFolhaPagamentoPeriodoMovimentacao->roRFolhaPagamentoFolhaComplementar->fecharFolhaComplementar();
        $stData = date("d/m/Y",strtotime(substr($obRFolhaPagamentoPeriodoMovimentacao->roRFolhaPagamentoFolhaComplementar->getTimestamp(),0,19)));
        SistemaLegado::LiberaFrames();
        if ( !$obErro->ocorreu() ) {
            //if ( $obRFolhaPagamentoPeriodoMovimentacao->roRFolhaPagamentoFolhaComplementar->roRFolhaPagamentoPeriodoMovimentacao->obRFolhaPagamentoFolhaSituacao->getSituacao() == 'Aberto' ) {
            //    SistemaLegado::alertaAviso($pgForm,"Folha Complementar Fechada em ".$stData.", a Folha Salário deverá ser recalculada.","incluir","aviso", Sessao::getId(), "../");
            //} else {
