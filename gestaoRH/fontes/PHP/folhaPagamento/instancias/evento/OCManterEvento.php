...
$Name$
$Author: souzadl $
$Date: 2008-03-27 17:23:22 -0300 (Qui, 27 Mar 2008) $
...
//function bloqueiaAbas($boExecuta = false) {
...
//    $stJs .= "window.parent.frames['telaPrincipal'].document.links['id_layer_5'].href = \"javascript:buscaValor('exibeAviso');\";\n";
//
//    if ($boExecuta) {
//        SistemaLegado::executaFrameOculto( $stJs );
//    } else {
...
{
//     $stJs .= ( !empty(Sessao::read('CasoSal')) ) ? montaListaCaso( Sessao::read('CasoSal') , 'Sal' ) : "";
//     $stJs .= ( !empty(Sessao::read('CasoFer')) ) ? montaListaCaso( Sessao::read('CasoFer') , 'Fer' ) : "";
//     $stJs .= ( !empty(Sessao::read('Caso13o')) ) ? montaListaCaso( Sessao::read('Caso13o') , '13o' ) : "";
//     $stJs .= ( !empty(Sessao::read('CasoRes')) ) ? montaListaCaso( Sessao::read('CasoRes') , 'Res' ) : "";
...
//    if ( $_POST['natureza'] == 'D' || $_POST['natureza'] == 'P' || $_POST['natureza'] == 'B' )
...
{
//    if ($_POST['natureza'] == 'P' or $_POST['natureza'] == 'D') {
        ...
        $obBscEvento->setId                            ( "inCampoInnerSal"                                           );
        $obBscEvento->obCampoCod->setName              ( "inCodigoSal"                                               );
        ...
        $obBscEvento->obCampoCod->obEvento->setOnChange( "buscaValor('preencherEventoBase');"                       );
        $obBscEvento->setFuncaoBusca                   ( "abrePopUp('".CAM_GRH_FOL_POPUPS."movimentacaoFinanceira/FLManterRegistroEvento.php','frm','inCodigoSal','inCampoInnerSal','','".Sessao::getId()."&boBase=true&boEventoSistema=false','800','550')" );

        $obBtnIncluir = new Button;
        $obBtnIncluir->setName                        ( "btnIncluirEventoBase"                                      );
        ...
        $obSpanEventosBaseCadastrados->setId          ( "spnEventosBaseSalCadastrados"                                 );

        $obFormulario = new Formulario;
        $obFormulario->addComponente                  ( $obBscEvento                                                );
        $obFormulario->agrupaComponentes              ( array($obBtnIncluir,$obBtnLimpar) );
//         $obFormulario->defineBarra                    ( array($obBtnIncluir,$obBtnLimpar),'CENTER',''               );
        $obFormulario->addSpan                        ( $obSpanEventosBaseCadastrados                               );
        $obFormulario->montaInnerHTML();
        $stHtmlSal = $obFormulario->getHTML();

        $obBscEvento->setId                           ( "inCampoInnerFer"                                              );
        $obBscEvento->obCampoCod->setName             ( "inCodigoFer"                                                  );
        $obBscEvento->setFuncaoBusca                  ( "abrePopUp('".CAM_GRH_FOL_POPUPS."movimentacaoFinanceira/FLManterRegistroEvento.php','frm','inCodigoFer','inCampoInnerFer','','".Sessao::getId()."&boBase=true&boEventoSistema=false','800','550')" );
        $obSpanEventosBaseCadastrados->setId          ( "spnEventosBaseFerCadastrados"                                 );
        $obFormulario = new Formulario;
        $obFormulario->addComponente                  ( $obBscEvento                                                );
        $obFormulario->agrupaComponentes              ( array($obBtnIncluir,$obBtnLimpar) );
//         $obFormulario->defineBarra                    ( array($obBtnIncluir,$obBtnLimpar),'CENTER',''                           );
        $obFormulario->addSpan                        ( $obSpanEventosBaseCadastrados                               );
        $obFormulario->montaInnerHTML();
        $stHtmlFer = $obFormulario->getHTML();

        $obBscEvento->setId                           ( "inCampoInner13o"                                              );
        $obBscEvento->obCampoCod->setName             ( "inCodigo13o"                                                  );
        $obBscEvento->setFuncaoBusca                  ( "abrePopUp('".CAM_GRH_FOL_POPUPS."movimentacaoFinanceira/FLManterRegistroEvento.php','frm','inCodigo13o','inCampoInner13o','','".Sessao::getId()."&boBase=true&boEventoSistema=false','800','550')" );
        $obSpanEventosBaseCadastrados->setId          ( "spnEventosBase13oCadastrados"                                 );
        $obFormulario = new Formulario;
        $obFormulario->addComponente                  ( $obBscEvento                                                );
        $obFormulario->agrupaComponentes              ( array($obBtnIncluir,$obBtnLimpar) );
//         $obFormulario->defineBarra                    ( array($obBtnIncluir,$obBtnLimpar),'CENTER',''                           );
        $obFormulario->addSpan                        ( $obSpanEventosBaseCadastrados                               );
        $obFormulario->montaInnerHTML();
        $stHtml13o = $obFormulario->getHTML();

        $obBscEvento->setId                           ( "inCampoInnerRes"                                              );
        $obBscEvento->obCampoCod->setName             ( "inCodigoRes"                                                  );
        $obBscEvento->setFuncaoBusca                  ( "abrePopUp('".CAM_GRH_FOL_POPUPS."movimentacaoFinanceira/FLManterRegistroEvento.php','frm','inCodigoRes','inCampoInnerRes','','".Sessao::getId()."&boBase=true&boEventoSistema=false','800','550')" );
        $obSpanEventosBaseCadastrados->setId          ( "spnEventosBaseResCadastrados"                                 );
        $obFormulario = new Formulario;
        $obFormulario->addComponente                  ( $obBscEvento                                                );
        $obFormulario->agrupaComponentes              ( array($obBtnIncluir,$obBtnLimpar) );
//         $obFormulario->defineBarra                    ( array($obBtnIncluir,$obBtnLimpar),'CENTER',''                           );
        $obFormulario->addSpan                        ( $obSpanEventosBaseCadastrados                               );
        $obFormulario->montaInnerHTML();
        $stHtmlRes = $obFormulario->getHTML();

//    } else {
//        $stHtml = "";
//    }
    $stJs .= "d.getElementById('spnEventoBaseSal').innerHTML = '".$stHtmlSal."'; \n";
    $stJs .= "d.getElementById('spnEventoBaseFer').innerHTML = '".$stHtmlFer."'; \n";
    $stJs .= "d.getElementById('spnEventoBase13o').innerHTML = '".$stHtml13o."'; \n";
    $stJs .= "d.getElementById('spnEventoBaseRes').innerHTML = '".$stHtmlRes."'; \n";
...
function incluirEventoBase()
{
    ...
    switch (Sessao::read('inAba')) {
        case 2:
            $rsEventosBase  = ( is_object(Sessao::read('eventosBaseSal')) ) ? Sessao::read('eventosBaseSal') : new recordset;
            $inCodigo       = $_POST['inCodigoSal'];
            $stDescricao    = $_POST['HdninCodigoSal'];
            $inCodConfiguracao = 1;
        break;
        case 3:
            $rsEventosBase  = ( is_object(Sessao::read('eventosBaseFer')) ) ? Sessao::read('eventosBaseFer') : new recordset;
            $inCodigo       = $_POST['inCodigoFer'];
            $stDescricao    = $_POST['HdninCodigoFer'];
            $inCodConfiguracao = 2;
        break;
        case 4:
            $rsEventosBase  = ( is_object(Sessao::read('eventosBase13o')) ) ? Sessao::read('eventosBase13o') : new recordset;
            $inCodigo       = $_POST['inCodigo13o'];
            $stDescricao    = $_POST['HdninCodigo13o'];
            $inCodConfiguracao = 3;
        break;
        case 5:
            $rsEventosBase  = ( is_object(Sessao::read('eventosBaseRes')) ) ? Sessao::read('eventosBaseRes') : new recordset;
            $inCodigo       = $_POST['inCodigoRes'];
            $stDescricao    = $_POST['HdninCodigoRes'];
            $inCodConfiguracao = 4;
        break;
    ...
    if ( !$obErro->ocorreu() ) {
        ...
        switch ( Sessao::read('inAba') ) {
            case 2:
                Sessao::write('eventosBaseSal',$rsEventosBase);
                $stJs .= "f.inCodigoSal.value = '';                                 \n";
                $stJs .= "d.getElementById('inCampoInnerSal').innerHTML = '&nbsp;'; \n";
            break;
            case 3:
                Sessao::write('eventosBaseFer',$rsEventosBase);
                $stJs .= "f.inCodigoFer.value = '';                                 \n";
                $stJs .= "d.getElementById('inCampoInnerFer').innerHTML = '&nbsp;'; \n";
            break;
            case 4:
                Sessao::write('eventosBase13o',$rsEventosBase);
                $stJs .= "f.inCodigo13o.value = '';                                 \n";
                $stJs .= "d.getElementById('inCampoInner13o').innerHTML = '&nbsp;'; \n";
            break;
            case 5:
                Sessao::write('eventosBaseRes',$rsEventosBase);
                $stJs .= "f.inCodigoRes.value = '';                                 \n";
                $stJs .= "d.getElementById('inCampoInnerRes').innerHTML = '&nbsp;'; \n";
            break;
        }
        $stJs .= gerarListaEventoBase();
    } else {
...
function excluirEventoBase()
{
    switch ( Sessao::read('inAba') ) {
        case 2:
            $rsEventosBase = Sessao::read('eventosBaseSal');
        break;
        case 3:
            $rsEventosBase = Sessao::read('eventosBaseFer');
        break;
        case 4:
            $rsEventosBase = Sessao::read('eventosBase13o');
        break;
        case 5:
            $rsEventosBase = Sessao::read('eventosBaseRes');
        break;
...
function gerarListaEventoBase()
{
    ...
    switch ( Sessao::read('inAba') ) {
        case 2:
            $rsEventosBase = Sessao::read('eventosBaseSal');
            $stIdSpan      = "spnEventosBaseSalCadastrados";
        break;
        case 3:
            $rsEventosBase = Sessao::read('eventosBaseFer');
            $stIdSpan      = "spnEventosBaseFerCadastrados";
        break;
        case 4:
            $rsEventosBase = Sessao::read('eventosBase13o');
            $stIdSpan      = "spnEventosBase13oCadastrados";
        break;
        case 5:
            $rsEventosBase = Sessao::read('eventosBaseRes');
            $stIdSpan      = "spnEventosBaseResCadastrados";
        break;
...
function preencherEventoBase()
{
    global $obRFolhaPagamentoEvento;
    switch ( Sessao::read('inAba') ) {
        case 2:
            $inCodigo = ( $_POST['inCodigoSal'] != "" ) ? $_POST['inCodigoSal'] : "";
        break;
        case 3:
            $inCodigo = ( $_POST['inCodigoFer'] != "" ) ? $_POST['inCodigoFer'] : "";
        break;
        case 4:
            $inCodigo = ( $_POST['inCodigo13o'] != "" ) ? $_POST['inCodigo13o'] : "";
        break;
        case 5:
            $inCodigo = ( $_POST['inCodigoRes'] != "" ) ? $_POST['inCodigoRes'] : "";
        break;
    }
    ...
    if ( $rsEventosBase->getNumLinhas() > 0 && $inCodigo != "" ) {
        switch ( Sessao::read('inAba') ) {
            case 2:
                $stJs .= "d.getElementById('inCampoInnerSal').innerHTML = '".$rsEventosBase->getCampo('descricao')."'; \n";
                $stJs .= "f.HdninCodigoSal.value = '".$rsEventosBase->getCampo('descricao')."';                 \n";
            break;
            case 3:
                $stJs .= "d.getElementById('inCampoInnerFer').innerHTML = '".$rsEventosBase->getCampo('descricao')."'; \n";
                $stJs .= "f.HdninCodigoFer.value = '".$rsEventosBase->getCampo('descricao')."';                 \n";
            break;
            case 4:
                $stJs .= "d.getElementById('inCampoInner13o').innerHTML = '".$rsEventosBase->getCampo('descricao')."'; \n";
                $stJs .= "f.HdninCodigo13o.value = '".$rsEventosBase->getCampo('descricao')."';                 \n";
            break;
            case 5:
                $stJs .= "d.getElementById('inCampoInnerRes').innerHTML = '".$rsEventosBase->getCampo('descricao')."'; \n";
                $stJs .= "f.HdninCodigoRes.value = '".$rsEventosBase->getCampo('descricao')."';                 \n";
            break;
        }
    } else {
        switch ( Sessao::read('inAba') ) {
...
function montaVerbaRescisoriaMTE($stNatureza)
{
    ...
    $stJs .= "d.getElementById('spnVerbaRescisoriaMTE').innerHTML = '".$stHTML."'; \n";
