...
function defineLayoutTermoRescisao(&$obPDF)
{
    ...
    $obPDF->Line( 17, 102, 203, 102 );
    $obPDF->Line( 17, 150, 203, 150 );
    $obPDF->Line( 17, 158, 203, 158 );
    ...
    //Discriminação das verbas rescisórias
    $obPDF->Line( 17 , 94 , 17 , 222 );
    $obPDF->Line( 107 , 94 , 107 , 222 );
...
function preencheTermoRescisao(&$obPDF,$arDados,$arEventos,$boPageBreak)
{
    ...
    //Dados do Contrato
    TextWithDirection($obPDF,11,91,'DADOS DO','U');
    TextWithDirection($obPDF,14,92,'CONTRATO','U');
    ...
    $obPDF->Text   ( 40,99,"VENCIMENTOS (29 A 45)" );
    $obPDF->Text   ( 134,99,"DESCONTOS (47 A 53)" );
    $obPDF->Text   ( 55,155,"BASES" );
    $obPDF->Text   ( 140,155,"INFORMATIVOS" );
    if (!$boPageBreak) {
    ...
    foreach ($arEventos["P"] as $arEventosProvento) {
        ...
        $obPDF->Text   ( 18, 106+$inEvento, $arEventosProvento["descricao"] );
        $obPDF->Text   ( 58, 106+$inEvento, $stDesdobramento );
        $obPDF->SetXY(74,103+$inEvento);
        $obPDF->Cell(20,4,number_format($arEventosProvento["quantidade"],2,',','.'),0,'R',0);
        $obPDF->SetXY(86,103+$inEvento);
    ...
    foreach ($arEventos["D"] as $arEventosDesconto) {
        ...
        $obPDF->Text   ( 18+90, 106+$inEvento, $arEventosDesconto["descricao"] );
        $obPDF->Text   ( 60+90, 106+$inEvento, $stDesdobramento );
        $obPDF->SetXY(80+90,103+$inEvento);
        $obPDF->Cell(20,4,number_format($arEventosDesconto["quantidade"],2,',','.'),0,'R',0);
        $obPDF->SetXY(92+90,103+$inEvento);
    ...
    foreach ($arEventos["B"] as $arEventosBase) {
        ...
        $obPDF->Text   ( 18, 162+$inEvento, $arEventosBase["descricao"] );
        $obPDF->Text   ( 58, 162+$inEvento, $stDesdobramento );
        $obPDF->SetXY(74,159+$inEvento);
        $obPDF->Cell(20,4,number_format($arEventosBase["quantidade"],2,',','.'),0,'R',0);
        $obPDF->SetXY(86,159+$inEvento);
    ...
    foreach ($arEventos["I"] as $arEventosInformativo) {
        ...
        $obPDF->Text   ( 18+90, 162+$inEvento, $arEventosInformativo["descricao"] );
        $obPDF->Text   ( 60+90, 162+$inEvento, $stDesdobramento );
        $obPDF->SetXY(80+90,159+$inEvento);
        $obPDF->Cell(20,4,number_format($arEventosInformativo["quantidade"],2,',','.'),0,'R',0);
        $obPDF->SetXY(92+90,159+$inEvento);
