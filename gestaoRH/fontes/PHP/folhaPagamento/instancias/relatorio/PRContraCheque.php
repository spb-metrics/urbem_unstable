...
/**
    ...
    * @ignore
    ...
    $Name$
    $Author: rgarbin $
    $Date: 2008-04-04 15:54:13 -0300 (Sex, 04 Abr 2008) $
...
$inCodMes = ($arFiltro["inCodMes"])?$arFiltro["inCodMes"]:$_POST["inCodMes"];
$inAno    = ($arFiltro["inAno"])?$arFiltro["inAno"]:$_POST["inAno"];
...
$stTipoFiltro = ($arFiltro["stTipoFiltro"])?$arFiltro["stTipoFiltro"]:$_POST["stTipoFiltro"];
$stOrdenacao = ($arFiltro["stOrdenacao"])?$arFiltro["stOrdenacao"]:$_POST["stOrdenacao"];
$stSituacao = ($arFiltro["stSituacao"])?$arFiltro["stSituacao"]:$_POST["stSituacao"];
...
$boAgrupar = ($arFiltro["boAgrupar"])?$arFiltro["boAgrupar"]:$_POST["boAgrupar"];
...
$boComplementar = ($arFiltro["boComplementar"])?$arFiltro["boComplementar"]:$_POST["boComplementar"];
$inCodConfiguracao = ($arFiltro["inCodConfiguracao"])?$arFiltro["inCodConfiguracao"]:$_POST["inCodConfiguracao"];
$inFolha = ($inCodConfiguracao != "") ? $inCodConfiguracao : 0;

$inCodComplementar   = ($arFiltro["inCodComplementar"])?$arFiltro["inCodComplementar"]:$_POST["inCodComplementar"];
$stDesdobramento     = ($arFiltro["stDesdobramento"])?$arFiltro["stDesdobramento"]:$_POST["stDesdobramento"];
$inContratoReemissao = ($arFiltro["inContratoReemissao"])?$arFiltro["inContratoReemissao"]:$_POST["inContratoReemissao"];
...
if ( (isset($arFiltro["boDuplicar"]))?$arFiltro["boDuplicar"]:$_POST["boDuplicar"] ) {
    $boDuplicar = 'true';
} else {
    $boDuplicar = 'false';
}
...
$obTOrcamentoEntidade->setDado("cod_entidade",Sessao::getCodEntidade());
...
$rsContratos->setCampo("competencia",$stCompetencia,true);
    ...
    $boMensagemAniversariante = ($arFiltro["boMensagemAniversariante"])?$arFiltro["boMensagemAniversariante"]:$_POST["boMensagemAniversariante"];
    $stMensagemAniversario = ($arFiltro["stMensagemAniversario"])?$arFiltro["stMensagemAniversario"]:$_POST["stMensagemAniversario"];
    $stMensagem = ($arFiltro["stMensagem"])?$arFiltro["stMensagem"]:$_POST["stMensagem"];
