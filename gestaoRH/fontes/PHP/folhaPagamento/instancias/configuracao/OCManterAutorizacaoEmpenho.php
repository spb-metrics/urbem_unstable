...
function validarEvento()
{
    ...
    if ($_REQUEST["inNumPAOEvento"] == "") {
        $obErro->setDescricao($obErro->getDescricao()."@Campo PAO inválido!()");
    }
    if ($_REQUEST["stMascClassificacao"] == "") {
...
function processarInformacoesEvento()
{
    Sessao::write("stOpcoesConfiguracaoEvento",$_REQUEST["stOpcoesConfiguracaoEvento"]);
    switch ($_REQUEST["stOpcoesConfiguracaoEvento"]) {
        case "lotacao";
        ...
        case "atributo";
            ...
            $obRPessoalServidor = new RPessoalServidor();
            $obRPessoalServidor->addContratoServidor();
            $obRPessoalServidor->roUltimoContratoServidor->obRCadastroDinamico->setChavePersistenteValores( array('cod_atributo'=>$_REQUEST['inCodAtributoEvento']) );
            $obRPessoalServidor->roUltimoContratoServidor->obRCadastroDinamico->recuperaAtributosSelecionados( $rsAtributos );

            $stNomeAtributo = "AtributoEvento_".$_REQUEST["inCodAtributoEvento"]."_".$_REQUEST["inCodCadastroEvento"];
            $arDados["inCodigo"] = $_REQUEST["inCodCadastroEvento"]."-".$_REQUEST["inCodAtributoEvento"];
            if ($rsAtributos->getCampo("cod_tipo") == 3 or $rsAtributos->getCampo("cod_tipo") == 4) {
                if ($rsAtributos->getCampo("cod_tipo") == 3) {
                    include_once(CAM_GA_ADM_MAPEAMENTO."TAdministracaoAtributoValorPadrao.class.php");
                    $obTAdministracaoAtributoValorPadrao = new TAdministracaoAtributoValorPadrao();
                    $stFiltro  = " WHERE cod_modulo = 22";
                    $stFiltro .= "   AND cod_cadastro = ".$_REQUEST["inCodCadastroEvento"];
                    $stFiltro .= "   AND cod_atributo = ".$_REQUEST["inCodAtributoEvento"];
                    $stFiltro .= "   AND cod_valor = ".$_REQUEST["$stNomeAtributo"];
                    $obTAdministracaoAtributoValorPadrao->recuperaTodos($rsValor,$stFiltro);
                    $arDados["stDescricao"] = $rsValor->getCampo("valor_padrao");
                    $arDados["extra"] = $_REQUEST[$stNomeAtributo];
                }
                if ($rsAtributos->getCampo("cod_tipo") == 4) {
                    $arDados["extra"] = $_REQUEST[$stNomeAtributo."_Selecionados"];
                    Sessao::write($stNomeAtributo."_Selecionados",true);
                }
            } else {
...
function incluirEvento()
{
    ...
    if (!$obErro->ocorreu()) {
        ...
        $arConfiguracaoEvento["stSituacao2"]                = $_REQUEST["stSituacao2"];
        $arConfiguracaoEvento["stSituacao3"]                = $_REQUEST["stSituacao3"];
        $arConfiguracaoEvento["stMascClassificacao"]        = $_REQUEST["stMascClassificacao"];
        $arConfiguracaoEvento["stRubricaDespesa"]           = buscaRubrica($_REQUEST["stMascClassificacao"]);
        $arConfiguracaoEvento["inCodDespesa"]               = $_REQUEST["inCodDespesa"];
        ...
        $arConfiguracaoEvento["inCodRegime"]                = $_REQUEST["inCodRegimeSelecionados"];
        $arConfiguracaoEvento["inCodSubDivisao"]            = $_REQUEST["inCodSubDivisaoSelecionados"];
