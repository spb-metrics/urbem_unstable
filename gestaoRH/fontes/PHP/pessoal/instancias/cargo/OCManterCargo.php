...
function addRegimeSubDivisao(&$obFormulario,$boCargo=true)
{
    ...
    if (Sessao::read("stAcao") == "alterar") {
        ...
        $obLista->Head->addCabecalho("Vagas Disponiveis"   ,10);
    }
...
function incluirEspecialidade()
{
    ...
    $arTemp["inId"]                     = count($arEspecialidades);
    $arTemp["stDescricaoEspecialidade"] = $_GET["stDescricaoEspecialidade"];
    $arTemp["inNumCBO"]                 = $_GET["inNumCBO"];
    $arTemp["inCodCBO"]                 = $_GET["inCodCBO"];
    $arTemp["stCBO"]                    = getDescricaoCBO($_GET["inCodCBO"]);
    $arTemp["inCodPadrao"]              = $_GET["inCodPadrao"];
    $arTemp["stPadrao"]                 = getDescricaoPadrao($_GET["inCodPadrao"]);
    $arTemp["stNorma"]                  = $_GET["stNorma"];
    $arTemp["stCodNorma"]               = $_GET["stCodNorma"];
...
function limparQuantidades($arVagas)
{
    ...
    foreach ($arVagas as $stCampo=>$inValor) {
        ...
        $stJs .= "jQuery('#btIncluirEspecialidade').attr('disabled','');\n";
        $stJs .= "jQuery('#btAlterarEspecialidade').attr('disabled','disabled');\n";
...
function montaAlterarEspecialidade()
{
    ...
    $stJs .= "jQuery('#btIncluirEspecialidade').attr('disabled','disabled');\n";
    $stJs .= "jQuery('#btAlterarEspecialidade').attr('disabled','');\n";
...
switch ($_GET["stCtrl"]) {
    ...
    case "incluirEspecialidade":
        $stJs = incluirEspecialidade();
        break;
    case "alterarEspecialidade":
        $stJs = alterarEspecialidade();
        break;
