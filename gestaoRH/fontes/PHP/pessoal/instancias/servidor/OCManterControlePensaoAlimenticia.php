...
function incluir($boExecuta = false)
{
    ...
    if ( count(Sessao::read('aPensoes')) > 0 ) {
        foreach ( Sessao::read('aPensoes') as $linha ) {
            if ($linha['cod_dependente']  == $_POST['inCodDependente']) {
                $boAchou = true;
            }
        }
    }
    ...
    if ($boAchou == true) {
        ...
        $obErro =  salvarRegistro(); // passando os dados pro array
        if ( !$obErro->ocorreu() ) {
            $stJs .= montaSpanLista( false )  ;
            $stJs .= limpar ( false );
            //sistemaLegado::exibeAviso('Pensão incluida com sucesso!','','');
        } else {
...
function alterarPensao($boExecuta = false)
{
    ...
    $arTemp = $arPensoes[$_GET['inId']];

    /// este array é usado como os objetos ocultos
    $arAlteracao['inId']          = $_GET['inId'];
    ...
    if ($arTemp ['tipo_pensao'] == 'Jurídica') {
        ...
        $stJs .= "d.frm.rdoTipoPensao[1].checked = true; \n ";
    }

    // combo de dependente
    $stJs .= "d.frm.txtCodDependente.value     = '". $arTemp['cod_dependente'] ."' ; \n";
    $stJs .= "f.inCodDependente.value          = '". $arTemp['cod_dependente'] ."' ; \n";
    ...
    $stJs .= "d.getElementById('txtOBS').value = '". $stOBS ."';" ;

    // preenchendo o combo de banco
    $stJs .= "d.frm.inCodBanco.value =  '". $arTemp['codBanco']   ."' ; \n ";
    $stJs .= "f.stBanco.value = '". $arTemp['codBanco']   ."' ; \n ";
    $stJs .= preencheAgenciaBancaria( $arTemp['codBanco'], false );
    // preenchendo a agencia
    $stJs .= "d.frm.inCodAgencia.value = '" .$arTemp['codAgencia']. "'; \n";
    $stJs .= "f.stAgenciaBanco.value   = '" .$arTemp['codAgencia']. "'; \n";

    $stJs .= "d.frm.dtDataInclusao.value = '" . $arTemp[ 'dataInclusao' ] . "'; \n";
    $stJs .= "d.frm.dtDataLimite.value =   '" . $arTemp[ 'dataLimite'   ] . "'; \n";
    $stJs .= "d.frm.stPercentual.value =   '" . $arTemp[ 'Percentual'   ] . "'; \n";

    if ($arTemp['valor']) {
        $stJs .= "d.frm.rdoTipoDesconto[0].checked = true; \n ";
        $stJs .= montaSpanValor(false);
        $stJs .= "d.frm.flValor.value          = '". $arTemp[ 'valor' ] . "'; \n";
    } else {
        $stJs .= "d.frm.rdoTipoDesconto[1].checked = true; \n ";
        $stJs .= montaSpanFuncao( false );
        $stJs .= "d.frm.inCodFuncao.value = '".$arTemp['codFuncao']."'; \n ";
        $stJs .= "d.getElementById('stFuncao').innerHTML = '". $arTemp['Funcao']. " '; \n";
    }

    $stJs .= "f.txtContaCorrente.value = '". $arTemp[ 'contaCorrente' ] . "'; \n";

    if ($arTemp['inNumCGMResp']) {
        $stJs .= montaSpanResponsavel( false );
        $stJs .= "d.frm.rdoResponsavel[1].checked = true; \n";
        $stJs .= "d.frm.inNumCGM.value            = '" . $arTemp['inNumCGMResp'] . "'; \n";
    ...
    $stJs .= "d.frm.chIncidencia_1.checked = ". ($arTemp [ 'chIncidencia_1'    ] ? "true" : "false")  ." ; \n";
    $stJs .= "d.frm.chIncidencia_2.checked = ". ($arTemp [ 'chIncidencia_2'    ] ? "true" : "false")  ." ; \n";
    $stJs .= "d.frm.chIncidencia_3.checked = ". ($arTemp [ 'chIncidencia_3'    ] ? "true" : "false")  ." ; \n";
    $stJs .= "d.frm.chIncidencia_4.checked = ". ($arTemp [ 'chIncidencia_4'    ] ? "true" : "false")  ." ; \n";
    $stJs .= "d.frm.chIncidencia_5.checked = ". ($arTemp [ 'chIncidencia_5'    ] ? "true" : "false")  ." ; \n";
    $stJs .= "d.frm.chIncidencia_6.checked = ". ($arTemp [ 'chIncidencia_6'    ] ? "true" : "false")  ." ; \n";

    //
    $stJs .= "d.frm.btnAlterar.disabled       = false; \n";
    $stJs .= "d.frm.btnIncluir.disabled       = true;  \n";
    $stJs .= "d.frm.txtCodDependente.disabled = true;  \n";
    $stJs .= "f.inCodDependente.disabled      = true;  \n";
...
function salvarPensao($boExecuta = false)
{
    ...
    if ( salvarRegistro($arAlteracao['inId'] ) ) {
...
function salvarRegistro($inId = '')
{
    ...
    if (($_POST[ 'rdoTipoDesconto' ] == 'F' ) and (!$_POST['inCodFuncao']) ) {
        $obErro->setDescricao('Informe a função a ser usada no cálculo!');
    } elseif ( (!$_POST [ 'inNumCGM' ] ) and ($_POST [ 'inNumCGM' ] == 'R') ) {
        $obErro->setDescricao('Informe o responsável pela conta corrente!');
    }

    if ( !$obErro->ocorreu() ) {
         $arRegistro[ 'tipo_pensao'     ] = ($_POST[ 'rdoTipoPensao' ] == 'J') ? 'Jurídica' : 'Amigavel';
         $arRegistro[ 'OBS'             ] = $_POST [ 'txtOBS'           ] ;
         $arRegistro[ 'Percentual'      ] = $_POST [ 'stPercentual'     ] ;
         $arRegistro[ 'dataInclusao'    ] = $_POST [ 'dtDataInclusao'   ] ;
         $arRegistro[ 'dataLimite'      ] = $_POST [ 'dtDataLimite'     ] ;

         $arRegistro[ 'codBanco'        ] = $_POST [ 'inCodBanco'       ] ;
         $arRegistro[ 'codAgencia'      ] = $_POST [ 'inCodAgencia'     ] ;
         $arRegistro[ 'contaCorrente'   ] = $_POST [ 'txtContaCorrente' ] ;

         if ($_POST[ 'rdoTipoDesconto' ] == 'V') {
             $arRegistro[ 'valor'          ] = $_POST [ 'flValor'          ] ;
         } else {
             ...
             $arRegistro[ 'evento'         ] = $_POST [ 'inCodFuncao'      ] . ' - '. $stNomeFuncao;
             $arRegistro[ 'Funcao'         ] = $stNomeFuncao;
             $arRegistro[ 'codFuncao'      ] = $_POST [ 'inCodFuncao'      ] ;
         }

         $arRegistro[ 'inNumCGMResp'   ] = $_POST [ 'inNumCGM'         ] ;

         $arRegistro[ 'chIncidencia_1'  ] = $_POST [ 'chIncidencia_1'    ] ;
         $arRegistro[ 'chIncidencia_2'  ] = $_POST [ 'chIncidencia_2'    ] ;
         $arRegistro[ 'chIncidencia_3'  ] = $_POST [ 'chIncidencia_3'    ] ;
         $arRegistro[ 'chIncidencia_4'  ] = $_POST [ 'chIncidencia_4'    ] ;
         $arRegistro[ 'chIncidencia_5'  ] = $_POST [ 'chIncidencia_5'    ] ;
         $arRegistro[ 'chIncidencia_6'  ] = $_POST [ 'chIncidencia_6'    ] ;

         if ($inId == '') {
             // inclusão de registro
             $obRPessoalServidor = new RPessoalServidor;
             $obRPessoalServidor->obRCGMPessoaFisica->setNumCGM ( $_POST['inCGM']                        );
             ...
             $obRDependente->setCodDependente        ( $_POST [ 'inCodDependente'] );
             ...
             $arRegistro[ 'cod_dependente' ] = $_POST [ 'inCodDependente'] ;
...
switch ($_POST["stCtrl"]) {
    ...
    case 'preencheAgenciaBancaria':
         preencheAgenciaBancaria($_POST['inCodBanco'], true );
    break;
    case 'incluir':
         incluir ( true );
    break;
