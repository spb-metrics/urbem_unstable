...
function adicionaRegistros(&$rsPensoes, &$obRDependente, &$obRPessoalPensao)
{
    global $_POST;
...
if ($_REQUEST['inNumCGM']) {
    $inCGM = $_REQUEST['inNumCGM'];
} else {
    // retiranto o cgm da strimg  $_POST['hdnCGM']
    $inCGM = trim( substr( $_POST['hdnCGM'], 0, ( strpos($_POST['hdnCGM'], '-') -1)) );
}
...
if ( $rsDependentes->getNumLinhas() <= 0 ) {
    ...
    adicionaRegistros($rsPensoes, $obRDependente, $obRPessoalPensao);
    ...
    $dtDataInclusao->setValue  ( $_POST['dtDataInclusao']                             );
    ...
    $dtDataLimite->setValue  ( $_POST['dtDataLimite']                 );
