...
/**
    ...
    * @ignore
    ...
    $Id: FMManterAssentamento.php 59612 2014-09-02 12:00:51Z gelson $
...
if ( empty($stAcao)||$stAcao=="incluir" ) {
    ...
    if ( $rsAssentamento->getCampo('cod_regime_previdencia') == 1 ) {
        $stRegimePrevidencia = "RGPS";
    } else {
        $stRegimePrevidencia = "RPPS";
    }
    $inCodRegimePrevidencia     = $rsAssentamento->getCampo('cod_regime_previdencia');
    $obRPessoalAssentamento->obRNorma->obRTipoNorma->listarTodos( $rsTipoNorma );
...
if ($_REQUEST['stAcao']=='alterar') {
   ...
   $obFormulario->addComponente    ( $obLblRegimePrevidencia    );
} else {
  $obFormulario->addComponente    ( $obTxtDescricao           );
  $obFormulario->addComponente    ( $obTxtSigla               );
  $obFormulario->addComponente    ( $obTxtAbreviacao               );
  $obFormulario->agrupaComponentes        ( array($obRdoRegimePrevidencia1,$obRdoRegimePrevidencia2));
}
