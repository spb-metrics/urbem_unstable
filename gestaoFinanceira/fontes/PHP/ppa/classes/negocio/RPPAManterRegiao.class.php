...
class RPPAManterRegiao
{
    ...
    {
        $obMapeamento = new TPPARegiao;
        $obMapeamento->proximoCod($_REQUEST['inCodRegiao']);
        $obMapeamento->setDado('cod_regiao', $_REQUEST['inCodRegiao']);
        $obMapeamento->setDado('nome', stripslashes($_REQUEST['stNome']));
        $obMapeamento->setDado('descricao', stripslashes($_REQUEST['stDescricao']));
    ...
    public function excluir($_REQUEST)
    {
        ...
        $stFiltro = ' where cod_regiao = ' . $_REQUEST['inCodRegiao'];
