...
class RPPAManterPPA
{
    ...
    public function excluir($arParametros)
    {
        ...
        $stFiltro = ' WHERE macro_objetivo.cod_ppa = '.$inCodPPA;
        $obErro = $obTPPAMacroObjetivo->recuperaTodos($rsMacroObjetivo, $stFiltro);
        ...
        if ($obErro->ocorreu()) {
            ...
            if ($rsMacroObjetivo->getNumLinhas() > 0) {
                return sistemaLegado::alertaAviso('LSManterPPA.php?stAcao=excluir','Não foi possível excluir o PPA pois esse PPA está vinculado a um Macro Objetivo.', 'n_excluir', 'aviso', Sessao::getId());
            } else {
