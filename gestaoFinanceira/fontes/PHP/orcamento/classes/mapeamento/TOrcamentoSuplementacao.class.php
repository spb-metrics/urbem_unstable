...
function recuperaRelacionamentoRecurso(&$rsRecordSet, $stCondicao = "", $stOrdem = "", $boTransacao = "")
{
    ...
    $stSql = $this->montaRecuperaRelacionamentoRecurso($boTransacao).$stCondicao.$stOrdem;
    $this->setDebug( $stSql );
...
function montaRecuperaRelacionamentoRecurso($boTransacao = "")
{
    ...
    $stSql .= "      ,TO_CHAR( OS.dt_suplementacao, 'dd/mm/yyyy' ) AS dt_suplementacao     \n";
    $stSql .= "      ,SUM(coalesce( OSS.valor, 0.00 )) AS vl_suplementado                  \n";
    $stSql .= "      ,SUM(coalesce( OSR.valor, 0.00 )) AS vl_reduzido                      \n";
    $stSql .= "FROM orcamento.suplementacao          AS OS                                 \n";
    $stSql .= "LEFT JOIN ( SELECT OSS.exercicio                                            \n";
    $stSql .= "                  ,OSS.cod_suplementacao                                    \n";
    $stSql .= "                  ,MAX( OSS.cod_despesa ) as cod_despesa                    \n";
    ...
    $stSql .= "                ,orcamento.recurso('".$this->getDado('stExercicio')."')  AS RECURSO            \n";
    $stSql .= "            WHERE                                                           \n";
    $stSql .= "                    OSS.cod_despesa = OD.cod_despesa                        \n";
    $stSql .= "                AND OSS.exercicio   = OD.exercicio                          \n";
    $stSql .= "                AND OD.cod_recurso  = RECURSO.cod_recurso                   \n";
    $stSql .= "                AND OD.exercicio    = RECURSO.exercicio                     \n";
    ...
    if ($boIsTCEMS) {
        ...
        $stSql .= "       AND OS.exercicio         = CTT.exercicio                          \n";
        $stSql .= " LEFT JOIN contabilidade.transferencia_despesa  AS CTD                   \n";
        $stSql .= "        ON OS.cod_tipo          = CTD.cod_tipo                           \n";
        $stSql .= "       AND OS.exercicio         = CTD.exercicio                          \n";
        $stSql .= "       AND OS.cod_suplementacao = CTD.cod_suplementacao                  \n";
        $stSql .= "     WHERE OSA.cod_suplementacao is null                                 \n";
    } else {
        $stSql .= "    ,contabilidade.tipo_transferencia     AS CTT                        \n";
        $stSql .= "    ,contabilidade.transferencia_despesa  AS CTD                        \n";
        $stSql .= "WHERE OS.cod_tipo          = CTT.cod_tipo                                   \n";
...
{
    $stSQL  .= "SELECT                                                                                            \n";
    $stSQL  .= "    S.cod_suplementacao ,                                                                         \n";
    $stSQL  .= "    S.exercicio ,                                                                                 \n";
    $stSQL  .= "    S.cod_norma ,                                                                                 \n";
    $stSQL  .= "    S.cod_tipo ,                                                                                  \n";
    $stSQL  .= "    TO_CHAR(S.dt_suplementacao,'dd/mm/yyyy') AS dt_suplementacao,                                 \n";
    $stSQL  .= "    TO_CHAR(SE.dt_suplementacao,'dd/mm/yyyy') AS dt_anulacao,                                     \n";
    $stSQL  .= "    S.motivo,                                                                                     \n";
    $stSQL  .= "    orcamento.fn_totaliza_suplementacao( S.exercicio, S.cod_suplementacao ) AS vl_suplementacao   \n";
    $stSQL  .= "FROM                                                                                              \n";
    $stSQL  .= "    orcamento.suplementacao S                                                                 \n";
    $stSQL  .= "    LEFT JOIN orcamento.suplementacao_anulada SA ON                                           \n";
    $stSQL  .= "    ( S.exercicio = SA.exercicio AND S.cod_suplementacao = SA.cod_suplementacao  )                \n";
    $stSQL  .= "    LEFT JOIN orcamento.suplementacao SE ON                                                   \n";
    $stSQL  .= "    ( SE.exercicio = SA.exercicio AND SA.cod_suplementacao_anulacao = SE.cod_suplementacao )      \n";
    $stSQL  .= "WHERE                                                                                             \n";
    $stSQL  .= "    S.cod_tipo <> 16                                                                              \n";
...
select suplementacao.exercicio
     ...
     , despesa.num_unidade
     , substr(despesa.num_pao, 1, 2) as tipo_acao
     , despesa.num_pao
