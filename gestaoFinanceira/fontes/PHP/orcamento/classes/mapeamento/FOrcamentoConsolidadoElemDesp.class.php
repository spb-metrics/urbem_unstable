    ...
    */
    ...
    public function montaRecuperaTodosTCEMG()
    {
        $stSql  = "SELECT *
                     FROM (
                        SELECT *                                                                                                    
                          FROM ".$this->getTabela()."('".$this->getDado("exercicio")."',' AND (cod_estrutural NOT ILIKE ''3.1.7.1%'' AND cod_estrutural NOT ILIKE ''3.3.7.1%'' AND cod_estrutural NOT ILIKE ''4.4.7.1%'') ".$this->getDado("stFiltro")." ','".$this->getDado("stDataInicial")."','".$this->getDado("stDataFinal")."','".$this->getDado("stEntidade")."','".$this->getDado("stCodOrgaoInicial")."','".$this->getDado("stCodOrgaoFinal")."','".$this->getDado("stCodUnidadeInicial")."','".$this->getDado("stCodUnidadeFinal")."','".$this->getDado('stDestinacaoRecurso')."','".$this->getDado('inCodDetalhamento')."', ".$this->getDado('inCodFuncao').", ".$this->getDado('inCodSubFuncao').") AS retorno (
                                 classificacao   varchar,
                                 cod_reduzido    varchar,
                                 descricao       varchar,
                                 num_orgao       integer,
