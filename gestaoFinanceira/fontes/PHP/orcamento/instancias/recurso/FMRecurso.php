...
if ($utilizado) {
    $obLblTipo = new Label;
    $obLblTipo->setRotulo( 'Tipo de Recurso' );
    $obLblTipo->setValue ( ($stTipo=='V')?'Vinculado':'Livre' );

    $obCampoTipo = $obLblTipo;

    $obHdnTipo = new Hidden;
    $obHdnTipo->setName ('stTipo');
    $obHdnTipo->setValue( $stTipo );

    $obLblFonteRecurso = new Label;
    $obLblFonteRecurso->setRotulo( 'Grupo Fonte de Recurso' );
    $obLblFonteRecurso->setValue ( $inCodFonteRecurso ." - ". $stNomFonteRecurso );

    $obCampoFonteRecurso = $obLblFonteRecurso;

    $obHdnFonteRecurso = new Hidden;
    $obHdnFonteRecurso->setName ( 'inCodFonteRecurso');
    $obHdnFonteRecurso->setValue( $inCodFonteRecurso );
}
...
if ($stAcao == 'alterar') {
    if ($utilizado) {
        ...
        $obFormulario->addHidden( $obHdnFonteRecurso );
    }
...
/*if (Sessao::getExercicio() > '2008') {
    $obFormulario->agrupaComponentes(array($obRdCriarContaContabilSim, $obRdCriarContaContabilNao));
}*/
