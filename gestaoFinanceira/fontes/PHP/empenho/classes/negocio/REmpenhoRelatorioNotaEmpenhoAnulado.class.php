...
<?php
 /**
    ...
    * @subpackage Regra
    ...
    $Name$
    $Author: vitor $
    $Date: 2007-07-30 18:38:44 -0300 (Seg, 30 Jul 2007) $
...
function geraRecordSet(&$arRecordSet , $stOrder = "")
{
    ...
    if ( !$rsRecordSet->eof() ) {
        ...
        if ($this->getReemitir() == "t") {

            $obTEmpenhoEmpenho->setDado( "cod_despesa", $rsRecordSet->getCampo('dotacao_reduzida') );
            $obTEmpenhoEmpenho->setDado( "timestamp", $this->getTimeStamp() );
            $obErro = $obTEmpenhoEmpenho->recuperaSaldoDotacaoData( $rsSaldoAnterior, $stOrder, $boTransacao );
            if ( !$obErro->ocorreu() ) {
                $nuSaldoAnterior = $arLinha6[0]['SaldoAnterior'] = $rsSaldoAnterior->getCampo( "saldo_anterior" );
            }
        } else $nuSaldoAnterior = $arLinha6[0]['SaldoAnterior'] = $this->getSaldoDotacao();
    ...
    if ($this->boImplantado != 't') {
        ...
        if ($this->getReemitir() == "t") {
            ...
            $arLinha6[0]['ValorOrcado']  = $nuValorOrcado;
            $arLinha6[0]['SaldoAnterior']= $this->getSaldoDotacao();
            $arLinha6[0]['ValorEmpenho'] = $nuValorEmpenho;
            $arLinha6[0]['SaldoAtual']   = $this->getSaldoDotacao() + $nuValorEmpenho;
        }
    }
