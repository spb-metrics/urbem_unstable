...
function geraRecordSet(&$arRecordSet , $stOrder = "")
{
    ...
    while ( !$rsRecordSet->eof() ) {
        if ( $stEmpenhoAtual != $rsRecordSet->getCampo('cod_empenho').'/'.$rsRecordSet->getCampo('exercicio_empenho') ) {
            ...
            $stDescricao = $rsRecordSet->getCampo('cod_empenho').'/'.$rsRecordSet->getCampo('exercicio_empenho');
        ...
        $arBloco4[ $inLinha   ]['1'] .= '    Liquidação: '. $rsRecordSet->getCampo('cod_nota').'/'.$rsRecordSet->getCampo('exercicio_nota').' - '.$rsRecordSet->getCampo('dt_liquidacao');
