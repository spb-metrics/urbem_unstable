    ...
    public function emitir($arParam)
    {
        ...
        if (!$obErro->ocorreu()) {
            ...
            case 'transferencia':
                ...
                $this->obModel->roRTesourariaBoletim->stExercicio = $arParam['stExercicio'];
                $this->obModel->obRContabilidadeLancamentoValor->obRContabilidadeLancamento->obRContabilidadeLote->obROrcamentoEntidade->inCodigoEntidade = $arParam['inCodEntidade'];
                $this->obModel->obRTesourariaAutenticacao->stTipo = $arParam['stTipo'];
            ...
            foreach ($arCheques as $arCheque) {
                if (!$obErro->ocorreu()) {
                    ...
                    if ($arParam['stTipoEmissaoCheque'] == 'ordem_pagamento') {
                        $obErro = $this->obModel->emitirPorOP();
                    } elseif ($arParam['stTipoEmissaoCheque'] == 'despesa_extra') {
                        $obErro = $this->obModel->emitirPorReciboExtra($arParam);
                    } else {
                        $obErro = $this->obModel->emitirPorTransferencia();
                    }
                    if ($obErro->ocorreu()) {
                        break;
                    }
    ...
    public function anularBaixaTransferencias($arParam)
    {
        ...
        foreach ($arParam as $stKey=>$stValue) {
            if (strstr($stKey,'transferencia')) {
                ...
                $this->obModel->obRContabilidadeLancamentoValor->bRContabilidadeLancamento->obRContabilidadeLote->obROrcamentoEntidade->inCodigoEntidade = $arTransferencia[2];
                $this->obModel->roRTesourariaBoletim->stExercicio = $arTransferencia[3];
                $this->obModel->obRTesourariaAutenticacao->stTipo = $arTransferencia[4];
