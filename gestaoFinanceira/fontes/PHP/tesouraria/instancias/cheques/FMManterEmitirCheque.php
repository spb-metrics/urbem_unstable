...
$pgList = 'LSManterEmitirCheque.php?stAcao=' . $_REQUEST['stAcao'];
...
$obHdnTipoEmissao->setValue   ($_REQUEST['stTipoEmissaoCheque']);
...
$obILabelEntidade->setExercicio       ($_REQUEST['stExercicio']  );
...
switch ($_REQUEST['stTipoEmissaoCheque']) {
case 'ordem_pagamento':
    ...
    $obTxtValorRetencao->setValue  ($_REQUEST['flValorRetencao']);
...
case 'transferencia':
    ...
    $obIPopUpContaBancoDebito->setValue             ($_REQUEST['stNomPlanoDebito']);
    $obIPopUpContaBancoDebito->obCampoCod->setValue ($_REQUEST['inCodPlanoDebito']);
    ...
    $obIPopUpContaBancoCredito->setValue             ($_REQUEST['stNomPlanoCredito']);
    $obIPopUpContaBancoCredito->obCampoCod->setValue ($_REQUEST['inCodPlanoCredito']);
...
case 'despesa_extra':
    ...
    $obTxtReciboExtra->setValue    ($_REQUEST['inCodReciboExtra']);
...
$obTxtCredor->setValue    ($_REQUEST['stNomCredor']);
$obTxtCredor->setLabel    (true                    );

if ($_REQUEST['stTipoEmissaoCheque'] == 'ordem_pagamento') {
    $flValorCheque = str_replace(',','.',str_replace('.','',$_REQUEST['flValor']));
    $flValorCheque -= str_replace(',','.',str_replace('.','',$_REQUEST['flValorRetencao']));
    $flValorCheque = number_format($flValorCheque,2,',','.');
} else {
    $flValorCheque = $_REQUEST['flValor'];
}
...
$obHdnDataCheque->setValue   ($_REQUEST['stDataCheque']);
...
$obDtEmissao->setValue             ($_REQUEST['stDataCheque']  );
if ($_REQUEST['stTipoEmissaoCheque'] == 'transferencia') {
...
$obIMontaContaCheque->setObrigatorioBarra   ();
$obIMontaContaCheque->setTipoBusca          ('naoEmitidos');
$obIMontaContaCheque->setVinculoPlanoBanco  (true);
$obIMontaContaCheque->setCodEntidadeVinculo ($_REQUEST['inCodEntidade']);
...
if ($_REQUEST['stTipoEmissaoCheque'] == 'despesa_extra') {
    $obValorCheque->setValue       ($_REQUEST['flValor']       );
