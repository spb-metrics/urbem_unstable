...
BEGIN
    ...
    stSql := '  SELECT  tabela.*
                ...
                LEFT JOIN ( SELECT publico.fn_mascara_completa(''' || stMascara || ''', plano_conta.cod_estrutural) as codigo_estrutural, *
                              FROM contabilidade.plano_conta
                             WHERE plano_conta.exercicio = ''' || stExercicio || '''
                          ) AS plano_conta                                        
                     ON plano_conta.codigo_estrutural = tabela.cod_estrutural
