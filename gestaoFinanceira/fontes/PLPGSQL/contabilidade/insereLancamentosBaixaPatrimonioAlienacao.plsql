...
/*
    ...
    * @subpackage 
    ...
    $Id: $
*/
CREATE OR REPLACE FUNCTION contabilidade.fn_insere_lancamentos_baixa_patrimonio_alienacao(INTEGER, INTEGER, INTEGER, VARCHAR, DATE, NUMERIC, INTEGER, VARCHAR, TIMESTAMP, INTEGER, VARCHAR, BOOLEAN) RETURNS VOID AS $$
DECLARE
    ...
    PstExercicioArrecadacao     ALIAS FOR $8;
    PstTimestampArrecadacao     ALIAS FOR $9;
    PinCodHistorico             ALIAS FOR $10;
    PstTipo                     ALIAS FOR $11;
    PboEstorno                  ALIAS FOR $12;
...
BEGIN
   ...
   INNER JOIN contabilidade.plano_analitica
           ON plano_analitica.exercicio  = plano_conta.exercicio 
          AND plano_analitica.cod_conta  = plano_conta.cod_conta 
        WHERE plano_conta.cod_estrutural = '7.2.1.1.1.00.01.00.00.00'
          AND plano_analitica.exercicio  = PstExercicio;

     IF inCodPlanoDebControle IS NULL THEN
        RAISE EXCEPTION 'Conta ( 7.2.1.1.1.00.01.00.00.00 ) não é analítica ou não está cadastrada no plano de contas.';
     END IF;
   ...
   INNER JOIN contabilidade.plano_analitica
           ON plano_analitica.exercicio  = plano_conta.exercicio 
          AND plano_analitica.cod_conta  = plano_conta.cod_conta 
        WHERE plano_conta.cod_estrutural = '8.2.1.1.1.00.01.00.00.00'
          AND plano_analitica.exercicio  = PstExercicio;

     IF inCodPlanoCredControle IS NULL THEN
        RAISE EXCEPTION 'Conta ( 8.2.1.1.1.00.01.00.00.00 ) não é analítica ou não está cadastrada no plano de contas.';
     END IF;
    ...
    LOOP
        ...
        IF PboEstorno = FALSE THEN
            ...
            VALUES
                ( reBaixaBemAlienacao.cod_bem, PdtDataBaixa, 'Baixa do Patrimônio por Alienação', inTipoGanhoAlienacao );
        ELSE
