...
--          coalesce(orcamento.fn_consolidado_empenhado('''''' || dataInicio || '''''', '''''' || dataFim || '''''', publico.fn_mascarareduzida(d.cod_estrutural),tbl.num_orgao,tbl.num_unidade),0.00) as empenhado_ano,
...
--          coalesce(orcamento.fn_consolidado_anulado('''''' || dataInicio || '''''', '''''' || dataFim || '''''', publico.fn_mascarareduzida(d.cod_estrutural),tbl.num_orgao,tbl.num_unidade),0.00) as anulado_ano,
...
--          (coalesce(orcamento.fn_consolidado_pago('''''' || dataInicio || '''''', '''''' || dataFim || '''''',publico.fn_mascarareduzida(d.cod_estrutural),tbl.num_orgao,tbl.num_unidade),0.00) - coalesce(orcamento.fn_consolidado_estornado('''''' || dataInicio || '''''', '''''' || dataFim || '''''', publico.fn_mascarareduzida(d.cod_estrutural),tbl.num_orgao,tbl.num_unidade),0.00)) as pago_ano,
...
--          (coalesce(orcamento.fn_consolidado_liquidado('''''' || dataInicio || '''''', '''''' || dataFim || '''''',publico.fn_mascarareduzida(d.cod_estrutural),tbl.num_orgao,tbl.num_unidade),0.00) - coalesce(orcamento.fn_consolidado_liquidado_estornado('''''' || dataInicio || '''''', '''''' || dataFim || '''''', publico.fn_mascarareduzida(d.cod_estrutural),tbl.num_orgao,tbl.num_unidade),0.00)) as liquidado_ano,
...
stSql := ''
    SELECT 
        ...
        ,CASE WHEN publico.fn_nivel(classificacao) <= 3 THEN
            orcamento.fn_consolidado_despesa_desdobramento(publico.fn_mascarareduzida(classificacao))
         ELSE
            saldo_inicial
         END as saldo_inicial
        ,CASE WHEN publico.fn_nivel(classificacao) <= 3 THEN
            orcamento.fn_consolidado_despesa_suplementacoes(publico.fn_mascarareduzida(classificacao))
         ELSE
            suplementacoes
         END AS suplementacoes
        ,CASE WHEN publico.fn_nivel(classificacao) <= 3 THEN
            orcamento.fn_consolidado_despesa_reducoes(publico.fn_mascarareduzida(classificacao))
         ELSE
            reducoes
         END as reducoes
        ,empenhado_per
