...
*
* $Id: gerarOrcamento.plsql 59767 2014-09-10 13:50:47Z michel $
*
...
BEGIN
   ...
   varProximoExercicio := BTRIM(TO_CHAR(TO_NUMBER(varExercicio,'99999') + 1, '9999'));
   ...
   ELSE
      ...
      SELECT CASE WHEN entidade.exercicio IS NOT NULL THEN 
                      true 
                  ELSE 
                      false 
             END AS bo_entidade INTO bolEntidadeReplicada 
    ...
    GROUP BY entidade.exercicio;
      ...
      LOOP
        ...
        --ELSE
          --  IF (recRegistroAux.cod_fonte = '') THEN
            --    UPDATE orcamento.recurso SET cod_fonte = recRegistro.cod_fonte::varchar, nom_recurso = recRegistro.nom_recurso::varchar WHERE exercicio = recRegistroAux AND cod_recurso = recRegistro.cod_recurso;
            --END IF;
        END IF;
      END LOOP;
      ...
      IF bolDespesa = true THEN
        stSql := 'SELECT DISTINCT
                        ...
                        JOIN orcamento.despesa_acao
                             ON despesa_acao.exercicio_despesa  = '|| quote_literal(varExercicio) ||'
        ...
        LOOP
            ...
            INSERT INTO orcamento.despesa (exercicio
                                            , cod_despesa
                                            , cod_conta
                                            , cod_entidade
                                            , cod_recurso
            ...
            --                                        
            INSERT INTO orcamento.despesa_acao (exercicio_despesa
                                                , cod_acao
                                                , cod_despesa
                                                ) VALUES ( varProximoExercicio
            ...
            IF bolPrevisaoDespesa = true THEN
              stSqlAux := 'SELECT DISTINCT
                              ...
                              JOIN orcamento.despesa
                                   ON despesa.exercicio      = despesa_acao.exercicio_despesa
                                  AND despesa.cod_despesa    = despesa_acao.cod_despesa
                              JOIN orcamento.previsao_despesa
