...
/*
$Log$
Revision 1.9  2007/09/03 14:58:39  vitor
Ticket#10014#

Revision 1.8  2007/08/28 15:54:16  vitor
Bug#10014#

Revision 1.7  2006/12/12 22:13:55  cleisson
Bug #7551#

Revision 1.6  2006/12/05 21:12:07  cleisson
Bug #7551#

Revision 1.5  2006/07/05 20:38:11  cleisson
...
--    stFiltroPag         := stFiltroPag          || '' AND  CPC.cod_estrutural like ''''1.1.1.%'''' '';
--    stFiltroPagEst      := stFiltroPagEst       || '' AND  CPC.cod_estrutural like ''''1.1.1.%'''' '';
--    stFiltroTransf      := stFiltroTransf       || '' AND  ( ( CPC.cod_estrutural like ''''1.1.2.%'''' AND CPCD.cod_estrutural like ''''1.1.2.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''1.1.3.%'''' AND CPCD.cod_estrutural like ''''1.1.3.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''1.2.1.%'''' AND CPCD.cod_estrutural like ''''1.2.1.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.1.1.%'''' AND CPCD.cod_estrutural like ''''2.1.1.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.1.2.%'''' AND CPCD.cod_estrutural like ''''2.1.2.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.1.9.%'''' AND CPCD.cod_estrutural like ''''2.1.9.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.2.1.%'''' AND CPCD.cod_estrutural like ''''2.2.1.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.2.2.%'''' AND CPCD.cod_estrutural like ''''2.2.2.%'''' ) ) '';
--
--    stFiltroTransfEst   := stFiltroTransfEst    || '' AND  ( ( CPC.cod_estrutural like ''''1.1.2.%'''' AND CPCD.cod_estrutural like ''''1.1.2.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''1.1.3.%'''' AND CPCD.cod_estrutural like ''''1.1.3.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''1.2.1.%'''' AND CPCD.cod_estrutural like ''''1.2.1.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.1.1.%'''' AND CPCD.cod_estrutural like ''''2.1.1.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.1.2.%'''' AND CPCD.cod_estrutural like ''''2.1.2.%'''' ) OR
...
--                                                             ( CPC.cod_estrutural like ''''2.2.1.%'''' AND CPCD.cod_estrutural like ''''2.2.1.%'''' ) OR
--                                                             ( CPC.cod_estrutural like ''''2.2.2.%'''' AND CPCD.cod_estrutural like ''''2.2.2.%'''' ) ) '';
--  stFiltroPagRest     := stFiltroPagRest      || '' AND  plano_banco.cod_estrutural like ''''1.1.1.%'''' '';
END IF;
...
'|| stCampos ||'
    ...
    --------------------------------------------------------
        SELECT
            ...
            ,OE.entidade
            ,ODE.cod_recurso
            ,CASE WHEN (emp.exercicio = to_char(TP.timestamp,''yyyy'')) THEN CAST(''O'' as varchar) 
    ...
    --BUSCA CONTA BANCO
            INNER JOIN (
                ...
                TP.cod_plano        = CPC.cod_plano AND
                TP.exercicio_plano  = CPC.exercicio 
            )
    --LIGAÇÃO COM NOTA_LIQUIDACAO_PAGA
            INNER JOIN empenho.nota_liquidacao_paga as NLP on(
                NLP.exercicio       = TP.exercicio      AND
                NLP.cod_nota        = TP.cod_nota       AND
                NLP.cod_entidade    = TP.cod_entidade   AND
                NLP.timestamp       = TP.timestamp      
            )
    ...
    --LIGAÇÃO COM EMPENHO
            INNER JOIN empenho.empenho as EMP on(
                EMP.exercicio       = NL.exercicio_empenho  AND
                EMP.cod_empenho     = NL.cod_empenho        AND
                EMP.cod_entidade    = NL.cod_entidade   
            )
    --LIGAÇÃO COM PRE_EMPENHO_DESPESA
            INNER JOIN empenho.pre_empenho_despesa as PED on(
                PED.exercicio       = EMP.exercicio         AND
                PED.cod_pre_empenho = EMP.cod_pre_empenho   
            )
    --LIGAÇÃO COM DESPESA
            INNER JOIN orcamento.despesa as ODE on(
                ODE.exercicio       = PED.exercicio     AND
                ODE.cod_despesa     = PED.cod_despesa   
            )
    --LIGAÇÃO COM RECURSO
            INNER JOIN (
                SELECT
                     ORE.nom_recurso as recurso
                    ,ORE.exercicio
                    ,ORE.cod_recurso
                ...
                ORE.exercicio    = ODE.exercicio     AND
                ORE.cod_recurso  = ODE.cod_recurso
    ';
...
if (inCodDetalhamento is not null and inCodDetalhamento <> '') then
    ...
    --LIGAÇÃO COM CONTA_DESPESA
            LEFT JOIN orcamento.conta_despesa as OCD on(
                OCD.exercicio       = PED.exercicio AND
    ...
    --BUSCA ENTIDADE
            INNER JOIN(
                ...
                OE.cod_entidade = TP.cod_entidade   AND
                OE.exercicio    = TP.exercicio 
            )
...
' || stFiltroPag || '
    ...
    --------------------------------------------------------
        ...
        FROM
            tesouraria.pagamento as TP
            INNER JOIN tesouraria.pagamento_estornado as TPE on(
                TPE.cod_entidade    = TP.cod_entidade   AND
                TPE.exercicio       = TP.exercicio      AND
                TPE.timestamp       = TP.timestamp      AND
                TPE.cod_nota        = TP.cod_nota           
            )
    --BUSCA CONTA BANCO
            INNER JOIN (
                SELECT
    ...
    --LIGAÇÃO COM NOTA_LIQUIDACAO_PAGA_ANULADA
            INNER JOIN empenho.nota_liquidacao_paga_anulada NLPA on(
                NLPA.exercicio          = TPE.exercicio         AND
                NLPA.cod_nota           = TPE.cod_nota          AND
                NLPA.cod_entidade       = TPE.cod_entidade      AND
                NLPA.timestamp          = TPE.timestamp         AND
                NLPA.timestamp_anulada  = TPE.timestamp_anulado 
            )
    --LIGAÇÃO COM NOTA_LIQUIDACAO
            INNER JOIN empenho.nota_liquidacao as NL on(
                NL.exercicio       = NLPA.exercicio      AND
                NL.cod_nota        = NLPA.cod_nota       AND
                NL.cod_entidade    = NLPA.cod_entidade   
            )
    --LIGAÇÃO COM EMPENHO
            INNER JOIN empenho.empenho as EMP on(
                EMP.exercicio       = NL.exercicio_empenho  AND
...
'|| stFiltroPagEst ||'
    ...
    -- BUSCA CONTA BANCO        
            INNER JOIN (
                ...
                TT.cod_plano_credito= CPC.cod_plano AND
                TT.exercicio        = CPC.exercicio 
                   ...
                   -- BUSCA CONTA DESPESA        
                           INNER JOIN (
                               ...
                               TT.cod_plano_debito = CPCD.cod_plano AND
                               TT.exercicio        = CPCD.exercicio 
                           )
     --BUSCA ENTIDADE
            INNER JOIN(
                SELECT 
...
UNION ALL
    ...
    --------------------------------------------------------
    SELECT
          plano.cod_plano_debito as plano
        , CPC.nom_conta as conta                
