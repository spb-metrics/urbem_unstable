...
function insereComprasNotaFiscalFornecedorOrdem($inCodNota)
{
    ...
    $obComprasNotaFiscalFornecedorOrdem->inclusao();
}
...
function insereAlmoxarifadoCatalogoItemMarca($valor)
{
    ...
    $obAlmoxarifadoCatalogoItemMarca->recuperaPorChave( $rsAlmoxarifadoCatalogoItemMarca );
    if ($rsAlmoxarifadoCatalogoItemMarca->getNumLinhas() <= 0 ) {
        $obAlmoxarifadoCatalogoItemMarca->inclusao();
    }
}
...
function insereAlmoxarifadoEstoqueMaterial($valor)
{
    ...
    $obAlmoxarifadoEstoqueMaterial->recuperaPorChave( $rsAlmoxarifadoEstoqueMaterial );
    if ($rsAlmoxarifadoEstoqueMaterial->getNumLinhas() <= 0) {
        $obAlmoxarifadoEstoqueMaterial->inclusao();
    }
}
...
function insereAlmoxarifadoLancamentoMaterial($inNumLancamento, $valor, $quantidade)
{
    ...
    $obAlmoxarifadoLancamentoMaterial->proximoCod($inCodLancamento);
    ...
    $obAlmoxarifadoLancamentoMaterial->inclusao();
...
function insereAlmoxarifadoLancamentoPerecivel($inCodLancamento, $valor, $inNumLotePerecivel)
{
    ...
    $obAlmoxarifadoLancamentoPerecivel->inclusao();
}
...
function insereAlmoxarifadoPerecivel($valor, $vlrPerecivel)
{
    ...
    $obAlmoxarifadoPerecivel->recuperaPorChave($rsPerecivel);
    if ( !$rsPerecivel->eof() ) {
        $obAlmoxarifadoPerecivel->alteracao();
    } else {
        $obAlmoxarifadoPerecivel->inclusao();
    }
}
        ...
        foreach ($arItensEntrada as $chave => $valor) {
            ...
            if (isset($arItensPerecivel[$valor['inCodItem']])) {
                ...
                foreach ($arItensPerecivel[$valor['inCodItem']] as $chv => $vlrPerecivel) {
                    ...
                    $inCodLancamento = insereAlmoxarifadoLancamentoMaterial($inNumLancamento, $valor, $vlrPerecivel['inQtdePerecivel']);
            ...
            if (isset($arItensAtributo[$valor['inCodItem']])) {
                foreach ($arItensAtributo[$valor['inCodItem']] as $chv => $vlrAtributo) {
                    ...
                    $obAlmoxarifadoAtributoEstoqueMaterialValor->inclusao();
                }
            }
        }
