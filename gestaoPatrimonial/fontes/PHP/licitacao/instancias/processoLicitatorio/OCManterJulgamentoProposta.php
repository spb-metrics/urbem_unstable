...
switch ($stCtrl) {
    ...
    case 'salvarAlteracaoDados':
        ...
        # Salva os dados ao clicar no SALVAR nos detalhes do fornecedor (por item, lote ou preço global).
        if (($inCodStatus == 1) && (empty($stJustificativa))) {
           $stJs .= "alertaAviso('Digite a justificativa para a desclassificação.','n_erro','erro','".Sessao::getId()."');\n";
        } else {
            if (is_array($arFornecedores)) {
                    ...
                    if ($registros['cgm_fornecedor'] == $inCodFornecedor) {
                        ...
                        if ($inCodStatus == 0) {
                            ...
                            if ($registros['ordem'] != 1) {
                                ...
                                # Quando o usuário estiver atualizando alguma informação do primeiro colocado.
                                if (empty($stJustificativa)) {
                                    # Força o usuário a colocar uma mensagem quando estiver atualizando o primeiro lugar.
                                    $stJs .= "alertaAviso('Digite a justificativa para o primeiro colocado.','n_erro','erro','".Sessao::getId()."');\n";
                                } else {
                                    $arFornecedores[$inCodChave][$inId]['status']        = ($inCodStatus == 0 ) ? 'classificado' : 'desclassificado';
