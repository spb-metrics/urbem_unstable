...
$locaisSelecionados = implode(',' , $arrLocaisSelecionados );
 ...
 Sessao::write('locaisSelecionados', $locaisSelecionados);
 Sessao::write('tipoRelatorio', $tipoRelatorio);
...
switch ($tipoRelatorio) {
    case "divergencia":
        $filtro = "
                      WHERE arquivo_coletora.timestamp Between to_date('".$stDataInicial."', 'dd/mm/yyyy') and to_date('".$stDataFinal."', 'dd/mm/yyyy')
                           AND arquivo_coletora_consistencia.status ='Divergente'
    ...
    case "naoLidos":
        $filtro = "
                WHERE arquivo_coletora.timestamp Between to_date('".$stDataInicial."', 'dd/mm/yyyy') and to_date('".$stDataFinal."', 'dd/mm/yyyy')
                      AND arquivo_coletora_consistencia.status <>'Não cadastrado'
             GROUP BY arquivo_coletora.codigo
                           , arquivo_coletora.nome  ";
      ...
      case "semDivergencia":
          $filtro = "
                      WHERE arquivo_coletora.timestamp Between to_date('".$stDataInicial."', 'dd/mm/yyyy') and to_date('".$stDataFinal."', 'dd/mm/yyyy')
                           AND arquivo_coletora_consistencia.status ='Sem divergência'
                 GROUP BY arquivo_coletora.codigo
                               , arquivo_coletora.nome  ";
}
