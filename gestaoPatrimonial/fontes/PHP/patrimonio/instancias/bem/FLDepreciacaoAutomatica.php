...
$inCompetenciaDepreciacao = sistemaLegado::pegaConfiguracao('competencia_depreciacao', 6);
...
if ($inCompetenciaDepreciacao == 1) {
    $obSlcCompentecia = new SelectMeses;
    $obSlcCompentecia->setName('inCompetencia');
    $obSlcCompentecia->setRotulo('Competência');
    $obSlcCompentecia->setTitle('Selecione a compentência em que ocorrerá a depreciação.');
    $obSlcCompentecia->setValue(date('n'));
    $obSlcCompentecia->setNull(false);
} else {
    $inCountCompetencia = 1;
    $stChaveCompetencia = '';

    for ($inCount = 1; $inCount<=12 ;$inCount++) {
        $stChaveCompetencia .= str_pad($inCount, 2, '0',STR_PAD_LEFT).',';
        if ($inCountCompetencia==$inCompetenciaDepreciacao) {
            $inCountCompetencia = 0;
            $stChaveCompetencia = substr($stChaveCompetencia,0, -1);
            if (strpos($stChaveCompetencia, date('m') ) !== true ) {
                $stValue = $stChaveCompetencia;
            }
            $arOrdinalDesc = each($arOrdinal);
            $arCompetencia[]=array('chave'=>$stChaveCompetencia,'descricao' => $arOrdinalDesc['value'].' '.$arDescCompetencia[$inCompetenciaDepreciacao]);
            $stChaveCompetencia='';
        }
        $inCountCompetencia++;
    }
    ...
    $rsCompetencia->preenche($arCompetencia);

    $obSlcCompentecia = new Select;
    $obSlcCompentecia->setName('inCompetencia');
    $obSlcCompentecia->setRotulo('Competência');
    $obSlcCompentecia->setTitle('Selecione a compentência em que ocorrerá a depreciação.');
    $obSlcCompentecia->setCampoId('chave');
    $obSlcCompentecia->setCampoDesc('descricao');
    $obSlcCompentecia->setValue($stValue);
    $obSlcCompentecia->preencheCombo($rsCompetencia);
    $obSlcCompentecia->setNull(false);
}
