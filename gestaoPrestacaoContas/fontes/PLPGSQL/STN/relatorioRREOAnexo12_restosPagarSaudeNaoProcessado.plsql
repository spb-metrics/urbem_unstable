...
CREATE OR REPLACE FUNCTION stn.fn_rreo_anexo12_restos_pagar_saude_nao_processado( varchar, varchar, varchar, integer ) RETURNS SETOF RECORD AS '
DECLARE
    stExercicio             ALIAS FOR $1;
    stDtFinal               ALIAS FOR $2;
    stCodEntidades    ALIAS FOR $3;
    inBimestre            ALIAS FOR $4;
    stDtInicial             VARCHAR   :='''';
    ...
    LOOP
        ...
        stSql := '' INSERT INTO tmp_empenhos
            SELECT
             ...
             FROM(
                SELECT
                    publico.fn_mascara_dinamica( '''''' || stMascara  || '''''', (ped_d_cd.num_orgao    ||''''''|| ponto ||''''''|| ped_d_cd.num_unidade  ||''''''|| ponto ||''''''|| ped_d_cd.cod_funcao   ||''''''|| ponto ||''''''|| ped_d_cd.cod_subfuncao||''''''|| ponto ||''''''|| ped_d_cd.cod_programa ||''''''|| ponto ||''''''|| ped_d_cd.num_pao      ||''''''|| ponto ||''''''|| replace(ped_d_cd.cod_estrutural,''''.'''', '''''''') ) ) as cod_estrutural,
                ...
                - (empenho.fn_empenho_liquidado( e.exercicio ,e.cod_empenho , e.cod_entidade ,'''''' || stDtInicialEmissao || '''''' ,'''''' || stDtFinalExercicioAnterior || '''''') - empenho.fn_empenho_estorno_liquidacao( e.exercicio ,e.cod_empenho ,e.cod_entidade ,''''''  || stDtInicialEmissao || '''''' ,''''''  || stDtFinalExercicioAnterior || ''''''  )) as inscritos,
                    ...
                    -- Valor pago
                    CASE WHEN ('''''' || inBimestre  || '''''' = 6 AND '''''' || stExercicio  || '''''' =  '''''' || stExercicioAtual  || '''''') THEN
                        0.00 
                    ELSE
                        (empenho.fn_empenho_pago( e.exercicio ,e.cod_empenho, e.cod_entidade,'''''' || stDtInicialAux    || '''''','''''' || stDtFinal      || '''''') - empenho.fn_empenho_estornado( e.exercicio,e.cod_empenho , e.cod_entidade ,'''''' || stDtInicialAux || '''''' ,'''''' || stDtFinal || ''''''  )) 
                    END as valor_pago,
                    ...
                    -- Valor Anulado
                     CASE WHEN ('''''' || inBimestre  || '''''' = 6 AND '''''' || stExercicio  || '''''' =  '''''' || stExercicioAtual  || '''''') THEN
                        0.00 
                    ELSE
                        empenho.fn_empenho_anulado( e.exercicio ,e.cod_empenho , e.cod_entidade,'''''' || stDtInicialAux || '''''','''''' || stDtFinal || '''''') 
                ...
                WHERE
                    e.exercicio         =cast(''||reReg.exercicio||'' as varchar)  AND 
