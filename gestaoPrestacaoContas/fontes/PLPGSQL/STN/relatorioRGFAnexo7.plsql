...
BEGIN
    ...
    SELECT 
        ...
        SUM(total_mes_10) +
        SUM(total_mes_11) +
        SUM(total_mes_12) AS vl_rcl 
    FROM stn.pl_total_subcontas ( ''' || stDtFim || ''' ) AS retorno (
        ordem          INTEGER,
        cod_conta      VARCHAR,
        nom_conta      VARCHAR, 
    ...
    stSQL := '
    --INSERT INTO tmp_rgf_retorno VALUES (1, -1, '''', NULL, NULL);
    INSERT INTO tmp_rgf_retorno VALUES (1, 0, ''DESPESA COM PESSOAL'', NULL, NULL); ';
    IF (nuRCL = 0) THEN
    ...
    -- Por enquanto os valores a seguir não tem no sistema os códigos estruturais então os valores ficarão 0,00 como registrado no RGF ANEXO 4
        --stSQL := stSQL || 'INSERT INTO tmp_rgf_retorno VALUES (4, 1, ''Operações de Crédito Externas e Internas'', ' || nuOCEI || ', ' || nuOCEI*100 || ' );';
    ...
    -- Por enquanto os valores a seguir não tem no sistema os códigos estruturais então os valores ficarão 0,00 omo registrado no RGF ANEXO 4
        --stSQL := stSQL || 'INSERT INTO tmp_rgf_retorno VALUES (4, 1, ''Operações de Crédito Externas e Internas'', ' || nuOCEI || ', ' || (nuOCEI/nuRCL)*100 || ' );';
        stSQL := stSQL || 'INSERT INTO tmp_rgf_retorno VALUES (4, 1, ''Operações de Crédito Externas e Internas'', 0.00 , 0.00 );';
