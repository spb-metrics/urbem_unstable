...
DECLARE
    ...
    nuRCL                   NUMERIC;
    nu_I_no_quadrimestre    NUMERIC;
    nu_I_ate_quadrimestre   NUMERIC;
    nu_II_no_quadrimestre   NUMERIC;
    nu_II_ate_quadrimestre  NUMERIC;
    nuPercent               NUMERIC;
    nuPercent2              NUMERIC;
    nuPercent3              NUMERIC;
    nuPercent4              NUMERIC;
...
-- # Tabela para os dados Balancete Receita Valores
  ...
  stSql := '
    INSERT INTO tmp_balancete_valores (cod_estrutural,receita, recurso , descricao, valor_previsto,valor_no_quadrimestre, valor_ate_quadrimestre,diferenca )                     
       select * 
...
-- # Sujeitas ao Limite para Fins de Contratação Externas + Internas(I)
    ...
    FETCH crCursor INTO nu_I_no_quadrimestre;
     CLOSE crCursor;
...
stSql :='
    ...
    ';
    OPEN  crCursor FOR EXECUTE stSql;
    FETCH crCursor INTO nu_I_ate_quadrimestre;
     CLOSE crCursor;

stSql := '
    INSERT INTO tmp_relatorio values(1,''SUJEITAS AO LIMITE PARA FINS DE CONTRATAÇÃO (I)'','||nu_I_no_quadrimestre||','||nu_I_ate_quadrimestre||',''N'',1);

-- #    Mobiliaria
    INSERT INTO tmp_relatorio values(2
                                                        ,''Mobiliaria''
                                                        ,(SELECT coalesce(sum(valor_no_quadrimestre),0.00) FROM tmp_balancete_valores WHERE cod_estrutural like ''2.1.1%'')
                                                        ,(SELECT coalesce(sum(valor_ate_quadrimestre),0.00) FROM tmp_balancete_valores WHERE cod_estrutural like ''2.1.1%'')
                                                        ,''N''
                                                        , 1);
...
-- #            Identificação das Operações de Crédito Internas
 /*   INSERT INTO tmp_relatorio (nivel,item,valor_no_quadrimestre,valor_ate_quadrimestre,linha,tabela)
        SELECT
            3         as nivel,
            descricao as item,
            coalesce(valor_no_quadrimestre,0.00),
            coalesce(valor_ate_quadrimestre,0.00),
            ''N'' as linha,
            1 as tabela
        FROM tmp_balancete_valores
...
/*stSql := '
    INSERT INTO tmp_relatorio values(1,''POR ANTECIPAÇÃO DA RECEITA (II)'','||nu_II_no_quadrimestre||','||nu_II_ate_quadrimestre||',''N'');

-- # Total das Operações de Crédito (I+II)
    INSERT INTO tmp_relatorio values(1,''TOTAL DAS OPERAÇÕES DE CRÉDITO (I+II)'','||nu_I_no_quadrimestre+nu_II_no_quadrimestre||','||nu_I_ate_quadrimestre+nu_II_ate_quadrimestre||',''S'');
    '; 
EXECUTE stSql; */
...
-- ## % das Operações de Crédito Internas e Externas sobre a RCL
    ...
    INSERT INTO tmp_relatorio values(1,''LIMITE GERAL DEFINIDO POR RESOLUÇÃO DO SENADO FEDERAL PARA AS OPERAÇÕES DE CRÉDITO INTERNAS E EXTERNAS '','||nuPercent3||','||nu_II_no_quadrimestre||',''S'',2);
    
    INSERT INTO tmp_relatorio values(1,''LIMITE DE ALERTA (inciso III §1° do art.59 daLRF) - 90%'',NULL,NULL,''S'',2);

    INSERT INTO tmp_relatorio values(1,''OPERAÇÕES DE CRÉDITO POR ANTECIPAÇÃO DA RECEITA ORÇAMENTÁRIA'','||nuPercent2||','||nu_II_no_quadrimestre||',''S'', 2);

    INSERT INTO tmp_relatorio values(1,''LIMITE DEFINIDO POR RESOLUÇÃO DO SENADO FEDERAL PARA AS OPERAÇÕES DE CRÉDITO POR ANTECIPAÇÃO DA RECEITA ORÇAMENTÁRIA'','||nuPercent4||','||nu_II_no_quadrimestre||',''S'', 2);
