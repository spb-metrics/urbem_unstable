...
--                AND evento.natureza <> ''B''
    ...
    stSql := '
                ...
                INNER JOIN folhapagamento'|| stEntidade ||'.periodo_movimentacao
                        ON periodo_movimentacao.dt_final between to_date('|| quote_literal(dtInicio) ||', ''yyyy-mm-dd'') AND to_date('|| quote_literal(dtFinal) ||', ''yyyy-mm-dd'')
                 LEFT JOIN (
                                   ...
                                   INNER JOIN (
                                                  SELECT max(timestamp) as timestamp
                                                   ...
                                                   WHERE periodo_movimentacao.dt_final between to_date('|| quote_literal(dtInicio) ||', ''yyyy-mm-dd'') AND to_date('|| quote_literal(dtFinal) ||', ''yyyy-mm-dd'')
                                                     AND contrato_servidor_conta_salario_historico.timestamp::VARCHAR <= timestampfechamentoperiodomovimentacao( periodo_movimentacao.cod_periodo_movimentacao, '||quote_literal(stEntidade) ||')
                              ...
                              INNER JOIN (
                                             SELECT max(timestamp) as timestamp
                                               ...
                                               FROM pessoal'|| stEntidade ||'.contrato_pensionista_conta_salario
                                                  , folhapagamento'|| stEntidade ||'.periodo_movimentacao
                                              WHERE periodo_movimentacao.dt_final between to_date('|| quote_literal(dtInicio) ||', ''yyyy-mm-dd'') AND to_date('|| quote_literal(dtFinal) ||', ''yyyy-mm-dd'')
                                                AND contrato_pensionista_conta_salario.timestamp::VARCHAR <= timestampfechamentoperiodomovimentacao( periodo_movimentacao.cod_periodo_movimentacao, '|| quote_literal(stEntidade) ||')
                ...
                INNER JOIN (
                              ...
                              INNER JOIN (
                                               SELECT cod_contrato
                                                 ...
                                                 FROM pessoal'|| stEntidade ||'.contrato_servidor_orgao
                                                    , folhapagamento'|| stEntidade ||'.periodo_movimentacao
                                                WHERE periodo_movimentacao.dt_final between to_date('|| quote_literal(dtInicio) ||', ''yyyy-mm-dd'') AND to_date('|| quote_literal(dtFinal) ||', ''yyyy-mm-dd'')
                                                  AND contrato_servidor_orgao.timestamp::VARCHAR <= timestampfechamentoperiodomovimentacao( periodo_movimentacao.cod_periodo_movimentacao, '||quote_literal(stEntidade) ||')
                              ...
                              INNER JOIN (
                                               SELECT cod_contrato
                                                 ...
                                                 FROM pessoal'|| stEntidade ||'.contrato_pensionista_orgao
                                                    , folhapagamento'|| stEntidade ||'.periodo_movimentacao
                                                WHERE periodo_movimentacao.dt_final between to_date('|| quote_literal(dtInicio) ||', ''yyyy-mm-dd'') AND to_date('|| quote_literal(dtFinal) ||', ''yyyy-mm-dd'')
                                                  AND contrato_pensionista_orgao.timestamp::VARCHAR <= timestampfechamentoperiodomovimentacao( periodo_movimentacao.cod_periodo_movimentacao, '|| quote_literal(stEntidade) ||')
                                             GROUP BY cod_contrato
                                                    , cod_periodo_movimentacao
                                         ) as max_contrato_orgao
                                      ON max_contrato_orgao.cod_contrato = cso.cod_contrato
                       ...
                       AND contrato_servidor_orgao.cod_periodo_movimentacao = periodo_movimentacao.cod_periodo_movimentacao
                 LEFT JOIN (
                              ...
                              LEFT JOIN monetario.banco
                                   ...
                                   JOIN (
                                            SELECT periodo_movimentacao.cod_periodo_movimentacao
                                             ...
                                             WHERE periodo_movimentacao.dt_final between to_date('|| quote_literal(dtInicio) ||', ''yyyy-mm-dd'') AND to_date('|| quote_literal(dtFinal) ||', ''yyyy-mm-dd'')
                                               AND configuracao_banrisul_conta.timestamp::VARCHAR <= timestampfechamentoperiodomovimentacao( periodo_movimentacao.cod_periodo_movimentacao, '''')
                                          GROUP BY periodo_movimentacao.cod_periodo_movimentacao
                                          ORDER BY periodo_movimentacao.cod_periodo_movimentacao
                                        ) AS interna
                                     ON interna.timestamp = configuracao_banrisul_conta.timestamp
                                   JOIN folhapagamento.periodo_movimentacao
                                     ON periodo_movimentacao.cod_periodo_movimentacao = interna.cod_periodo_movimentacao
--                                   WHERE banco.num_banco = ''041''
    ...
    FOR reContratos IN EXECUTE stSql LOOP
        stFiltro     := ' AND cod_contrato             = '|| reContratos.cod_contrato             ||'
                          AND cod_periodo_movimentacao = '|| reContratos.cod_periodo_movimentacao ||'
                            ;
                        ';
