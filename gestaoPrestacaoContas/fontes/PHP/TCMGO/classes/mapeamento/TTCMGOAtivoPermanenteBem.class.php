...
/**
    ...
    * @subpackage Mapeamento
    ...
    $Id: TTCMGOAtivoPermanenteBem.class.php 61667 2015-02-24 13:46:37Z jean $
                  ...
                  SELECT * FROM (
                      ...
                      SELECT  bem.cod_bem
                         ...
                         , CASE WHEN bem.cod_natureza = 1 THEN '01'
                           ...
                           , CASE WHEN bem.cod_natureza = 1 THEN
                                bem_veiculo.tipo_bem_movel
                              ELSE '00'
                             END AS tipo_bem_movel 
                         
                           , CASE WHEN bem.cod_natureza = 1 AND bem_veiculo.tipo_bem_movel != '05' AND bem_veiculo.tipo_bem_movel != '99' THEN
                            ...
                            , 1 AS qtde
                            , bem.vl_bem AS vl_aquisicao_saldo_anterior
                            , bem_comprado_empenho.cod_empenho AS nro_empenho
                            , bem_comprado_empenho.exercicio   AS ano_empenho
                            , bem.vl_bem AS vl_aquisicao
                            , 0 AS vl_doacao
                            , COALESCE(max_reavaliacao.vl_reavaliacao, 0) AS vl_reavaliacao
                            , 0 AS vl_baixa_alienacao
                            , 0 AS vl_baixa_doacao
                            , 0 AS vl_baixa_depreciacao
                            , (SELECT vl_atualizado FROM patrimonio.fn_depreciacao_acumulada(bem.cod_bem) 
                                                      AS ( cod_bem INTEGER
                        ...
                        INNER JOIN orcamento.unidade
                         ...
                         LEFT JOIN ( SELECT   b.cod_bem
                             ...
                             INNER JOIN (SELECT MAX(timestamp) as timestamp, cod_veiculo
                                           FROM frota.veiculo_propriedade
                                       GROUP BY cod_veiculo) AS veiculo_propriedade
                    ...
                    WHERE resultado.tipo_bem_movel IS NOT NULL ";
        return $stSql;
    }
